...
function updateAttention(snapshot, previous, interventions) {
    const items = attentionSignals(snapshot).map((signal) => {
...
export function evaluateControl(input, previous) {
    ...
    const accepted = new Set(matching
        .filter((entry) => entry.resolution === 'accept')
        .flatMap((entry) => previous?.attention.find((item) => item.id === entry.itemId)?.riskIds ?? []));
    const graph = refreshGraph(input);
    const risk = assessRisk(input, graph.nodes, accepted);
    const decision = decideAutonomy(input, risk, {
        stopped: (Boolean(input.stopSignature) && !accepted.has('stop')) ||
...
export function markAttentionRead(state, at) {
...
export function resolveAttention(state, input, at) {
