...
test('allow executes synchronously, timeout records unknown and identical retries do not execute again', async (t) => {
  // The deadline includes schema validation. Freeze it until the fixture confirms
  // beforeCall has actually admitted the effect; wall-clock scheduling is not evidence.
  ...
  const pending = f.broker.call(input);
  await effectStarted;
  assert.equal(f.calls.length, 1);
...
test('deadline before the effect boundary records cancelled rather than unknown and never dispatches on retry', async (t) => {
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const f = fixture(t, { timeoutMs: 300 });
  f.policy('allow');
  const { job } = f.claim(),
  ...
  const stoppedBeforeEffect = new Promise((resolve) => {
    f.manager.invoke = async (...args) => {
      admitted();
      await gate;
      try {
  ...
  assert.equal((await f.broker.call(input)).status, 'cancelled');
  release();
  await stoppedBeforeEffect;
  assert.equal(f.calls.length, 0);
});
