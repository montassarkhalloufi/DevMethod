...
export function resolveFreshDestination(destination, pathApi = path) {
  if (
    ...
    throw new Error('Destination must be a safe absolute fresh path');
  const windows = pathApi.sep === '\\';
  const segments = destination.split(windows ? /[\\/]/ : '/');
  if (segments.includes('..') || (!windows && destination.includes('\\')))
    throw new Error('Destination must be a safe absolute fresh path');
  if (windows) {
    ...
    if (
      (!/^[a-z]:\\/i.test(native) && !/^\\\\[^\\]+\\[^\\]+/.test(native)) ||
      /^\\\\[?.]\\/.test(native)
    )
      throw new Error('Destination must use a fully qualified native path');
    const root = pathApi.parse(native).root;
    if (native.slice(root.length).includes(':'))
      throw new Error('Alternate stream paths are not allowed');
  }
  return pathApi.resolve(destination);
}

function readFixtureInputs(fixtureDirectory, fixture) {
  const prepared = [];
  for (const [name, source] of Object.entries(fixture.files)) {
    safeRelative(name);
    safeRelative(source.source);
    if (/(^|\/)(oracle[^/]*|expected[^/]*|manifest\.json)$/i.test(name))
      throw new Error('Evaluator-only artifact must not enter handoff');
    if (!/^[a-f0-9]{64}$/.test(source.sha256)) throw new Error('Invalid pinned digest');
    const sourcePath = path.join(fixtureDirectory, source.source);
    noLinks(sourcePath);
    const stat = fs.statSync(sourcePath);
    if (!stat.isFile() || stat.size > 1024 * 1024)
      throw new Error('Fixture source must be a regular file <= 1 MiB');
...
export function prepareBehaviorCase(
  caseId,
  destination,
  { fixtureDirectory = path.join(root, 'fixtures'), caseFile = path.join(root, 'cases.json') } = {},
) {
  destination = resolveFreshDestination(destination);
  noLinks(destination, true);
  if (fs.existsSync(destination))
    throw new Error('Destination already exists; use a fresh directory');
  fixtureDirectory = path.resolve(fixtureDirectory);
  noLinks(fixtureDirectory);
  noLinks(caseFile);
  ...
  if (
    ...
    throw new Error('Destination cannot alter fixture sources');
  ...
  const selected = manifest.cases.filter((c) => c.id === caseId);
  const specs = suite.cases.filter((c) => c.id === caseId);
  ...
  if (
    ...
    !Object.keys(fixture.files).length
  )
    throw new Error('Missing fixture files');
  const prepared = readFixtureInputs(fixtureDirectory, fixture);
  // Parent directories must already exist so preparation cannot accidentally build an unexpected hierarchy.
  if (!fs.statSync(path.dirname(destination)).isDirectory())
    throw new Error('Destination parent must exist');
  ...
  return {
    ...
    destination,
    prompt: spec.prompt,
    promptSha256: digest(spec.prompt),
    suiteSha256: digest(casesBytes),
    ...
    operatorSteps: fixture.operatorSteps,
    status: 'prepared-not-run',
    limitation:
      'Preparation establishes pinned input bytes only. Operator setup, actual host execution and independent adjudication remain required. Do not give this complete evaluator record to the agent; send prompt and fixture workspace only.',
  };
}
if (
  process.argv[1] &&
  ...
  try {
    ...
    if (!caseId || !destination || extra.length)
      throw new Error(
        'Usage: node scripts/prepare-behavior-case.mjs CASE_ID /absolute/fresh/destination',
      );
