...
export function snapshot(root) {
    ...
    function visit(directory, prefix) {
        for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
            ...
            if (!entry.isFile())
                throw new Error('Type de fichier refusé.');
            ...
            total += bytes.length;
            if (total > MAX_BYTES)
                throw new Error('Limite de compilation : 32 Mio.');
...
export function publish(root, outputs) {
    realDirectory(path.dirname(root));
    if (fs.existsSync(root))
        throw new Error('Le dossier de sortie doit être nouveau.');
    ...
    try {
        for (const output of outputs) {
            if (!safeRelative(output.path))
                throw new Error('Chemin compilé invalide.');
