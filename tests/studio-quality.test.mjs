...
test('catalogue does not invent success; absent tool and irrelevant stack are distinct', async (t) => {
  ...
  assert.equal(report.checks.find((c) => c.id === 'business-browser').status, 'notapplicable');
...
test('cancelled quality requests preserve a blocked receipt and never promote a late successful adapter result', async (t) => {
  const { store, id } = fixture(t);
  const before = new AbortController();
  before.abort();
  const unopened = t.mock.method(qualityAdapters, 'syntax', async () => {
    throw new Error('An already cancelled check must not call the adapter');
  });
  let report = await runProjectQuality(store, id, 'source-syntax', { signal: before.signal });
  assert.equal(unopened.mock.callCount(), 0);
  assert.equal(report.checks.find((row) => row.id === 'source-syntax').status, 'blocked');
  unopened.mock.restore();
  const during = new AbortController();
  t.mock.method(qualityAdapters, 'syntax', async () => {
    during.abort();
    return { status: 'passed', observed: 'Late controlled result', findings: [] };
  });
  report = await runProjectQuality(store, id, 'source-syntax', { signal: during.signal });
  const row = report.checks.find((row) => row.id === 'source-syntax');
  assert.equal(row.status, 'blocked');
  assert.match(row.evidence.observed, /interrompu/);
