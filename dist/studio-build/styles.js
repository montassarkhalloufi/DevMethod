...
function checkDirectives(css) {
    if (/@(?:plugin|config|source)\b/i.test(css))
        throw new Error('Les directives Tailwind @plugin, @config et @source externes ne sont pas exécutées dans Studio.');
    if (/@import\s+(?:url\s*\(|["'](?:[a-z]+:|\/\/))/i.test(css))
        throw new Error('Les imports CSS réseau ou url() ne sont pas pris en charge.');
}
export async function compileStyles(css, file, context) {
    ...
    const compiled = await compile(css, {
        ...
        loadStylesheet: async (id, base) => {
            ...
            else if (inside(tailwindRoot, base) && id.startsWith('.'))
                resolved = path.resolve(base, id);
            else if (id.startsWith('.') || id.startsWith('@/'))
                resolved = context.local(id, path.join(base, '_style.css'));
            else
                throw new Error(`Import CSS non autorisé : ${id}`);
            const content = context.read(resolved);
            if (content === undefined)
                throw new Error(`Feuille de style absente : ${id}`);
    ...
    if (compiled.sources.length || (compiled.root && compiled.root !== 'none'))
        throw new Error('Une source Tailwind externe est refusée. Les classes sont recherchées dans le seul instantané.');
