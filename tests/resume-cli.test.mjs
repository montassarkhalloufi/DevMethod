...
function fixture(t) {
  const destination = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), 'devmethod-resume-cli-'),
  );
  ...
  const save = () =>
    fs.writeFileSync(path.join(destination, 'checkpoint.json'), JSON.stringify(checkpoint));
  save();
  const run = (args = []) =>
    spawnSync(
      process.execPath,
      [cliPath, 'resume', '--dest', destination, '--checkpoint', 'checkpoint.json', ...args],
      { encoding: 'utf8' },
    );
...
test('resume CLI JSON is read-only and fails closed for all nonready statuses', (t) => {
  ...
  const before = fs
    ...
    .map((name) => [name, fs.readFileSync(path.join(destination, name), 'hex')]);
  let result = run(['--json']);
  ...
  checkpoint.nextAction = 'Resolve missing environment';
  save();
  result = run(['--json']);
  ...
  result = run(['--json']);
  assert.equal(result.status, 1);
  assert.equal(JSON.parse(result.stdout).status, 'reverify');
...
test('resume CLI rejects irrelevant flags and missing checkpoint, and refuses unsafe paths', (t) => {
  ...
  for (const args of [['--tool', 'codex'], ['--modules', 'scoped-delivery'], ['--dry-run']])
    assert.equal(run(args).status, 2);
  for (const args of [
    ['resume'],
    ['resume', '--checkpoint', ''],
    ['doctor', '--checkpoint', 'checkpoint.json'],
    ['update-preview', '--checkpoint', 'checkpoint.json'],
    ['init', '--tool', 'codex', '--checkpoint', 'checkpoint.json'],
  ]) {
    assert.equal(
      spawnSync(process.execPath, [cliPath, ...args], { encoding: 'utf8', cwd: destination })
        .status,
      2,
    );
  }
  for (const checkpoint of ['../outside.json', '/etc/passwd']) {
    const result = spawnSync(
      process.execPath,
      [cliPath, 'resume', '--dest', destination, '--checkpoint', checkpoint, '--json'],
      { encoding: 'utf8' },
    );
    assert.equal(result.status, 1);
    assert.equal(JSON.parse(result.stdout).status, 'invalid');
  ...
  const result = spawnSync(
    process.execPath,
    [cliPath, 'resume', '--checkpoint', 'checkpoint.json', '--json'],
    { encoding: 'utf8', cwd: destination },
  );
  assert.equal(result.status, 0);
