...
for (const tool of Object.keys(tools))
  test(`install ${tool}: complete payload, links, hashes and license`, (t) => {
    ...
    for (const name of modules) {
      ...
      for (const [, link] of content.matchAll(/\]\(([^)]+)\)/g)) {
        if (!link.includes('://') && !link.startsWith('#'))
          assert.ok(fs.existsSync(path.join(directory, link.split('#')[0])), link);
      }
    }
    assert.equal(
      fs.readFileSync(path.join(destination, 'DEVMETHOD-LICENSE'), 'utf8'),
...
test('dry-run does not write and subsets retain foundation', (t) => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.join(destination, tools.claude)).sort(),
    [
      'project-foundation',
      'scoped-delivery',
      ...commandSkills(['project-foundation', 'scoped-delivery']),
    ].sort(),
  );
});
...
test('conflicts fail before writing and project instructions stay intact', (t) => {
  ...
  for (const name of ['PROJECT_PROFILE.md', 'AGENTS.md', 'CLAUDE.md', 'LICENSE', 'package.json'])
    fs.writeFileSync(path.join(destination, name), 'existing');
  ...
  for (const name of ['AGENTS.md', 'CLAUDE.md', 'LICENSE', 'package.json'])
    assert.equal(fs.readFileSync(path.join(destination, name), 'utf8'), 'existing');
});
...
test('reject invalid arguments, duplicate hosts, symbolic and blocked paths', (t) => {
  ...
  assert.throws(
    () => initialize({ destination, tool: 'codex', selected: ['../../bad'] }),
    /Unknown module/,
  );
  initialize({ destination, tool: 'codex' });
  ...
  assert.throws(
    () => initialize({ destination: path.join(root, 'link'), tool: 'codex' }),
    /Symbolic/,
  );
  fs.writeFileSync(path.join(root, 'file'), 'keep');
  assert.throws(
    () => initialize({ destination: path.join(root, 'file', 'child'), tool: 'codex' }),
    /directory/,
  );
});
...
test('CLI validates noninteractive input and executes a real installation', (t) => {
  ...
  for (const args of [['init'], ['other'], ['init', '--tool', 'bad'], ['init', '--oops']])
...
for (const tool of Object.keys(tools))
  test(`workflow resources on ${tool}: complete references and mission preservation`, (t) => {
    const destination = fixture(t);
    ...
    for (const [skill, files] of Object.entries(resources))
    ...
    assert.deepEqual(
      [legacy, ticket].map((f) => fs.readFileSync(path.join(destination, f))),
      before,
    );
    assert.deepEqual(fs.readdirSync(path.join(destination, 'docs/missions')).sort(), [
      'current',
      'legacy.md',
    ]);
