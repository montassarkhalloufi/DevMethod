...
test('source-only snapshots never announce executed controls or a runnable preview', async (t) => {
  ...
  assert.equal(button('Préparer et examiner les sources').disabled, false);
});
...
test('creating a source-only candidate keeps valid text distinct from the temporary editing lock', async (t) => {
  ...
  const { editor, input, root, button } = setup(
    t,
    {
      ...
      apply: async () => ({
        ...
        draft: draft('', 2, { sourceOnly: true, files, preparedRevisionId: 'candidate' }),
      }),
    },
    {
      onApplied: () =>
  ...
  button('Préparer et examiner les sources').click();
  await setImmediate();
  assert.equal(editor.getBaseRevision(), 'base');
  ...
  assert.equal(input.disabled, false);
  assert.equal(fileMessage.textContent, '');
  assert.match(root.querySelector('[role=status]').textContent, /Version préparée pour examen/);
});
...
test('code changes survive a save in flight and only the newest acknowledged text becomes buildable', async (t) => {
  ...
  assert.equal(button('Préparer et examiner cette version').disabled, true);
  await editor.flush();
  assert.equal(saved[1].version, 2);
  assert.equal(saved[1].changes[0].content, 'newest');
  assert.equal(builds[0].version, 3);
  assert.equal(button('Préparer et examiner cette version').disabled, false);
});
...
test('manual preparation uses the exact built draft and a later keystroke disables it again', async (t) => {
  let applied;
  const { editor, type, button } = setup(t, {
    ...
    apply: async (payload) => {
      ...
      return {
        ...
        draft: { ...draft('ready', 8), preparedRevisionId: 'new' },
      };
    },
  });
  await editor.open('base');
  button('Préparer et examiner cette version').click();
  await setImmediate();
...
test('preparation locks typing until the response so an intermediate keystroke cannot be discarded', async (t) => {
  let release;
...
test('preparation hands the exact candidate to review while keeping the previous editing base', async (t) => {
  let examined;
  const prepared = draft('ready', 8, {
    buildId: 'built',
    builtVersion: 8,
    changedPaths: ['app.js'],
    preparedRevisionId: 'candidate',
  });
  const { editor, button, input } = setup(
    t,
    {
      read: async () => prepared,
      apply: async () => ({
  ...
  button('Préparer et examiner cette version').click();
  await setImmediate();
  assert.equal(examined.revision.id, 'candidate');
  assert.equal(editor.getBaseRevision(), 'base');
  assert.equal(input.value, 'ready');
});

test('activation reread merges browser edits made during the request and migrates recovery storage', async (t) => {
  let release;
  let reads = 0;
  const { editor, type, input, dom } = setup(t, {
    read: async () =>
      ++reads === 1
        ? draft('ready')
        : new Promise((resolve) => {
  ...
  await editor.open('base');
  type('before review');
  const pending = editor.reconcileActivation('candidate');
  assert.equal(input.disabled, false);
  type('during reread');
  release(
    draft('ready', 2, {
      ...
      files: [
        { path: 'app.js', content: 'ready', editable: true },
        { path: 'server.js', content: 'concurrent server edit', editable: true },
      ],
    }),
  );
  await pending;
  assert.equal(input.value, 'during reread');
  assert.equal(editor.getBaseRevision(), 'candidate');
  assert.equal(dom.window.localStorage.getItem('devmethod-code-draft:base'), null);
  assert.match(dom.window.localStorage.getItem('devmethod-code-draft:candidate'), /during reread/);
  editor.selectFile('server.js');
  assert.equal(input.value, 'concurrent server edit');
});

test('activation reread waits for a save in flight and preserves the subsequent keystroke', async (t) => {
  let releaseSave;
  let reads = 0;
  const { editor, type, input } = setup(t, {
    read: async () =>
      ++reads === 1 ? draft('ready') : draft('saved', 3, { baseRevision: 'candidate' }),
    save: () =>
      new Promise((resolve) => {
        releaseSave = resolve;
      }),
  ...
  const reconciling = editor.reconcileActivation('candidate');
  type('later keystroke');
  assert.equal(reads, 1);
  releaseSave(draft('saved', 2));
  await saving;
  await reconciling;
  assert.equal(reads, 2);
  assert.equal(input.value, 'later keystroke');
  assert.equal(editor.getBaseRevision(), 'candidate');
});

test('failed activation reconciliation preserves the base, text and recovery without retry', async (t) => {
  let reads = 0;
  const { editor, type, input, root, dom } = setup(t, {
    read: async () => {
      if (++reads === 1) return draft('ready');
      throw Object.assign(new Error('Contexte modifié'), { status: 409 });
    },
  });
  await editor.open('base');
  type('keep local');
  await editor.reconcileActivation('candidate');
  assert.equal(reads, 2);
  assert.equal(editor.getBaseRevision(), 'base');
  assert.equal(input.value, 'keep local');
  assert.match(root.querySelector('[role=status]').textContent, /conservées/);
  assert.match(dom.window.localStorage.getItem('devmethod-code-draft:base'), /keep local/);
});

test('switching FR/EN with an open unsaved draft preserves code, selection and editor identity', async (t) => {
  let saves = 0;
  const f = setup(t, {
    read: async () => draft(),
    save: async () => {
  ...
  await f.editor.open('base');
  const code = 'export const texte = "Brouillon français — do not translate";';
  f.type(code);
  f.input.focus();
  f.input.setSelectionRange(13, 18, 'forward');
  assert.ok(f.button('Vérifier et actualiser'));
  setLocale('en', f.dom.window.document, f.dom.window);
  assert.ok(f.button('Check and refresh'));
  assert.equal(f.root.querySelector('textarea'), f.input);
  assert.equal(f.input.value, code);
  assert.equal(f.input.selectionStart, 13);
  assert.equal(f.input.selectionEnd, 18);
  assert.equal(f.dom.window.document.activeElement, f.input);
