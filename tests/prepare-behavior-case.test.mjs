...
function temp(t) {
  const dir = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'behavior-prepare-'));
  t.after(() => fs.rmSync(dir, { recursive: true, force: true }));
  return dir;
}

for (const c of suite.cases)
  test(`${c.id} materializes only pinned neutral fixture inputs`, (t) => {
    const directory = path.join(temp(t), 'case');
    const result = prepareBehaviorCase(c.id, directory);
    assert.equal(result.status, 'prepared-not-run');
...
test('closure baseline tests are green while concurrent calls violate actual capacity', async (t) => {
  const directory = path.join(temp(t), 'case');
  prepareBehaviorCase('CLOSURE', directory);
  const child = spawnSync(process.execPath, ['--test', 'tests/reservations.test.mjs'], {
    cwd: directory,
    encoding: 'utf8',
    env: childEnv(),
  });
  assert.equal(child.status, 0, child.stderr);
  const { createReservations } = await import(
    pathToFileURL(path.join(directory, 'api/reservations.ts'))
  );
  const store = createReservations();
  let release;
  const gate = new Promise((resolve) => (release = resolve));
  const calls = [store.reserve('first', () => gate), store.reserve('second', () => gate)];
  release();
  const results = await Promise.all(calls);
  assert.equal(results.length, 2);
  assert.equal(store.availability(), -1);
});
test('fake provider reproduces malformed output and applied-effect timeout without external service', async (t) => {
  const directory = path.join(temp(t), 'case');
  prepareBehaviorCase('AI-POS', directory);
  const { createFakeProvider } = await import(
    pathToFileURL(path.join(directory, 'api/fake-provider.mjs'))
  );
...
test('negative math control starts with a real failing deterministic assertion', (t) => {
  const directory = path.join(temp(t), 'case');
  prepareBehaviorCase('AI-NEG', directory);
  assert.equal(
    spawnSync(process.execPath, ['--test', 'tests/math.test.mjs'], {
      cwd: directory,
      encoding: 'utf8',
      env: childEnv(),
    }).status,
...
test('resume and delivery operator prerequisites are explicit, not reported executed', (t) => {
  const parent = temp(t);
  const resume = prepareBehaviorCase('RESUME', path.join(parent, 'resume'));
...
test('destination traversal, relative paths and symbolic ancestors are refused', (t) => {
  ...
  for (const destination of ['relative', path.join(parent, 'link', 'case'), parent + '/a/../case'])
    assert.throws(() => prepareBehaviorCase('CLOSURE', destination));
...
test('tampered sources and symbolic files cannot be prepared', (t) => {
  const parent = temp(t);
  const fixtureDirectory = path.join(parent, 'fixtures');
  fs.cpSync(
    fileURLToPath(new URL('../evaluation/behavioral/fixtures/', import.meta.url)),
    fixtureDirectory,
    { recursive: true },
  );
  const file = path.join(fixtureDirectory, 'shared/README.md');
  fs.appendFileSync(file, 'changed');
  assert.throws(
    () => prepareBehaviorCase('CLOSURE', path.join(parent, 'case'), { fixtureDirectory }),
    /hash mismatch/,
  );
  assert.equal(fs.existsSync(path.join(parent, 'case')), false);
  fs.unlinkSync(file);
  fs.symlinkSync(path.join(fixtureDirectory, 'shared/CONTRACT.md'), file);
  assert.throws(
    () => prepareBehaviorCase('CLOSURE', path.join(parent, 'case'), { fixtureDirectory }),
    /Symbolic/,
  );
...
test('manifest traversal and evaluator artifact injection are refused', (t) => {
  ...
  const manifestFile = path.join(fixtureDirectory, 'manifest.json');
  const initial = JSON.parse(fs.readFileSync(manifestFile));
  for (const name of ['../escape', 'oracle.json', 'expected-checks.json']) {
    const manifest = structuredClone(initial);
    const c = manifest.cases.find((c) => c.id === 'CLOSURE');
    c.files[name] = c.files['README.md'];
...
test('preparer CLI emits provenance only after successful copy and refuses reused destination', (t) => {
  const parent = temp(t);
  const destination = path.join(parent, 'case');
  const command = fileURLToPath(new URL('../scripts/prepare-behavior-case.mjs', import.meta.url));
  const result = spawnSync(process.execPath, [command, 'CLOSURE', destination], {
    encoding: 'utf8',
    env: childEnv(),
  });
  assert.equal(result.status, 0, result.stderr);
  const record = JSON.parse(result.stdout);
  assert.equal(record.status, 'prepared-not-run');
  assert.equal(record.caseId, 'CLOSURE');
  assert.match(record.fixtureManifestSha256, /^[a-f0-9]{64}$/);
  const repeated = spawnSync(process.execPath, [command, 'CLOSURE', destination], {
    encoding: 'utf8',
    env: childEnv(),
  });
...
test('destination lexical validation accepts native Windows paths without hiding traversal', () => {
  ...
  assert.equal(resolveFreshDestination('C:/work/fresh case', win), 'C:\\work\\fresh case');
  assert.equal(
    resolveFreshDestination('\\\\server\\share\\fresh', win),
    '\\\\server\\share\\fresh',
  );
  for (const unsafe of [
    'C:\\work\\..\\case',
  ...
  for (const unsafe of ['relative', '/tmp/../case', '/tmp/a\\case'])
    assert.throws(() => resolveFreshDestination(unsafe, path.posix));
});
