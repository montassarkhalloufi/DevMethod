...
Status: proposed pending the user's opening-mode choice, 2026-09-13.
...
The user was invited to choose, propose another option or explicitly delegate. That choice is still pending; silence is not acceptance. No server or final automatic opening behavior has been selected. The browser renderer and report export remain useful under either mode.

Browser verification found and corrected a real focus-loss defect. Direct file:// reopening is blocked by the browser tool's URL policy; that limitation remains recorded, without a workaround or a claim of successful offline-browser reopening. A final release requires reconciling the opening mode and its remaining verification, then the authorized repository gates.
