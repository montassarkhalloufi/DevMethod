...
export function reviewOpenCommand(
  file: string,
  ...
  throw new Error(
    `Automatic review opening is unavailable on ${platform}; open the generated HTML in your browser.`,
  );
}
export function openReview(file: string): void {
  ...
  if (result.error || result.status !== 0)
    throw new Error(
      `The browser could not be opened. The generated report is preserved at ${file}; open it manually.`,
    );
}
