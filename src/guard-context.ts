...
export interface GuardSnapshot extends BehaviorInputs {
  format: 1;
...
function engineSignature(): string {
  const files = [
...
function artifactPaths(report: unknown, artifactPath: string): string[] {
  if (!object(report) || !Array.isArray(report.runs)) throw new Error('Invalid behavioral report.');
  return report.runs.flatMap((run: unknown) => {
    if (!object(run) || !Array.isArray(run.artifacts))
      throw new Error('Invalid behavioral artifacts.');
    return run.artifacts.map((artifact: unknown) => {
...
function snapshotFiles(root: string, paths: string[]): GuardSnapshot['files'] {
  const selected = [...new Set(paths)].sort();
  if (selected.length > 512) throw new Error('Guard context exceeds 512 declared files.');
  let total = 0;
  return selected.map((file) => {
    if (secretPath(file)) throw new Error('Secret-like guard context is excluded.');
    const bytes = readLocal(root, file, 8 * 1024 * 1024);
    total += bytes.length;
    if (total > 32 * 1024 * 1024) throw new Error('Guard context exceeds 32 MiB.');
...
export function captureGuardSnapshot(root: string, inputs: BehaviorInputs): GuardSnapshot {
  const selected = {
  ...
  const signature = digest(
    JSON.stringify({ ...context, files: files.map(({ path, sha256 }) => ({ path, sha256 })) }),
  );
