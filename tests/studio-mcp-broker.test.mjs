...
test('native session uses the real broker, preserves exact-call deduplication and cannot approve ask', async (t) => {
  const f = fixture(t);
  f.broker.select({ connectionIds: [f.id] });
  const claimed = f.claim();
  ...
  const post = async (route, input) => {
    const response = await fetch(new URL(route, session.url), {
      method: 'POST',
  ...
  const client = new Client({ name: 'native-integration-fixture', version: '1.0.0' });
  await client.connect(
    new StdioClientTransport({
      command: process.execPath,
      args: [fileURLToPath(new URL('../scripts/studio/native-tools-stdio.mjs', import.meta.url))],
      env: { DEVMETHOD_NATIVE_TOOLS_URL: session.url, DEVMETHOD_NATIVE_TOOLS_TOKEN: session.token },
      stderr: 'pipe',
    }),
  );
  t.after(() => client.close());
  assert.equal((await client.callTool({ name: 'studio_call', arguments: input })).isError, false);
  assert.equal((await post('/call', input)).status, 200);
  assert.equal(f.calls.length, 1);
  f.manager.permission = () => 'ask';
  const pending = await post('/call', { ...input, requestId: randomUUID() });
  assert.equal(pending.body.status, 'pending');
  assert.equal(
    (await post('/actions/decide', { requestId: pending.body.requestId, decision: 'allow' }))
      .status,
    404,
  );
  assert.equal(f.calls.length, 1);
  const actions = await post('/actions', { requestId: pending.body.requestId });
