...
import { validateGuardState } from './guard-state.js';
...
function sessionDirectory(root, session, create) {
    ...
    if (!fs.statSync(directory).isDirectory())
        throw new Error('Expected a guard session directory.');
    return directory;
}
function writeAtomic(directory, file, data) {
...
export function withGuardStore(root, session, create, action) {
    ...
    try {
        ...
        const state = fs.existsSync(statePath)
            ? validateGuardState(parseJson(readLocal(directory, 'state.json', MAX_STATE_BYTES).toString('utf8')))
            : null;
        return action({
            state,
