...
export function diagnose(destination: string): DoctorReport {
  const report: DoctorReport = { format: 1, destination: path.resolve(destination), status: 'ok', checked: 0, unchanged: 0, findings: [] };
  const add = (severity: Finding['severity'], code: string, message: string, file?: string) => {
    report.findings.push({ severity, code, ...(file === undefined ? {} : { path: file }), message });
    if (severity === 'error' || report.status === 'ok') report.status = severity;
  };
  ...
  for (const [relative, expected] of Object.entries(manifest.files)) {
    ...
    try {
      ...
      checkPath(file);
      const info = stat(file);
      if (!info) { add('error', 'file-missing', 'Restore from a reviewed staging installation.', relative); continue; }
      if (!info.isFile()) { add('error', 'file-type', 'Expected a regular file.', relative); continue; }
      const actual = createHash('sha256').update(fs.readFileSync(file)).digest('hex');
      if (actual === expected.toLowerCase()) report.unchanged++;
      else add('warning', 'file-modified', 'Differs from the initial installation. Review intentional customization; nothing was overwritten.', relative);
    } catch (error) {
  ...
  for (const name of manifest.skills) {
    for (const root of Object.values(tools)) {
      ...
      try {
        const file = path.join(report.destination, relative);
        checkPath(file);
        if (stat(file)) add('error', 'duplicate-host', 'Another host contains this module. Keep one reviewed copy per project.', relative);
      } catch (error) {
