  ...
  onEvent(event) {
    ...
    if (
      ...
      cancellation = Promise.all(
        [...turns].map(([threadId, turnId]) => rpc.call('turn/interrupt', { threadId, turnId })),
      );
      cancellation.catch((e) => done('interrupt-error: ' + e.message));
    }
    if (event.method === 'turn/completed') {
      completed.add(p.threadId);
      if (p.threadId === root) done(p.turn.status);
    }
  },
});
const abort = () => {
...
try {
  await rpc.call('initialize', {
    clientInfo: { name: 'devmethod_probe', version: '0.1.0' },
  ...
  const result = await rpc.call('thread/start', {
    ...
    developerInstructions:
      'Infrastructure probe in a fictional empty directory. No file writes, network tools, external actions or purchases. Only the single requested child and its explicitly requested sleep command are authorized.',
  });
    ...
    input: [
      {
        type: 'text',
        text: cancelling
          ? 'Spawn exactly one child to run the shell command sleep 30 and then answer CHILD_OK. Wait for the child and answer PARENT_OK. No other work or tools. This bounded delegation is authorized.'
          : 'Spawn exactly one child agent to answer CHILD_OK without using tools or spawning agents. Wait for that child to finish, then answer PARENT_OK. This bounded delegation is explicitly authorized.',
        text_elements: [],
      },
    ],
  });
  ...
  try {
    status = await Promise.race([
      ...
      new Promise((_, reject) => {
        deadline = setTimeout(() => reject(new Error('Probe deadline')), 110000);
      }),
    ]);
  } finally {
...
const record = {
  status: status === 'completed' || (cancelling && status === 'interrupted') ? 'exited' : 'failed',
  probeStatus: status,
  ...
  note: 'Observed per-thread totals; native accounting semantics still require review.',
};
