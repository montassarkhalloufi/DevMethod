...
function runCompiler(input, timeoutMs, signal) {
    return new Promise((resolve, reject) => {
        const child = spawn(process.execPath, ['--max-old-space-size=512', fileURLToPath(new URL('./worker.js', import.meta.url))], {
...
export async function buildReactApp(options) {
    const result = {
        ok: false,
    ...
    try {
        if (options.signal?.aborted)
            throw new Error('Compilation annulée.');
        const sourceRoot = realDirectory(options.sourceRoot), outputRoot = realDirectory(options.outputRoot);
