...
// Presentation is separate from applicationOptions used to validate and prepare launches.
export function projectHomeLaunchCatalog(locale = 'en') {
  const catalog = projectConnectorCatalog(locale);
  return {
    options: catalog.options.filter((entry) => entry.purpose === 'application'),
    capabilities: catalog.capabilities.filter((entry) => entry.purpose === 'application'),
  };
}
