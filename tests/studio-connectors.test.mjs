...
function fixture(t, source = 'const broken = ;') {
  ...
  const deliver = (content) => {
    ...
    return jobs.finish({ jobId: claim.job.id, title: 'Fixture', summary: 'Real file fixture' })
      .revision.id;
  };
  const revisionId = deliver(source);
  t.after(() => {
    store.close();
...
test('catalogue offers diagnostic tools and independent application providers without claiming connection', (t) => {
  const f = fixture(t),
...
test('every catalogue option can be configured and application capabilities prepare without external execution', (t) => {
  const f = fixture(t),
...
test('configuration preserves secret references only and stale changes cannot overwrite it', (t) => {
  const f = fixture(t),
    version = f.store.read().version;
...
test('MCP probe records an authenticated-host declaration and schema digests, never a check result', (t) => {
  const f = fixture(t);
...
test('a late older probe cannot replace the newest observed connection state', (t) => {
  const f = fixture(t);
  configured(f);
  const first = probe(f);
...
test('probe replay is idempotent, conflicting replay fails, and config changes invalidate it', (t) => {
  const f = fixture(t);
  configured(f);
  const input = probe(f),
...
test('tickets require an applicable attested tool and do not execute or register success', (t) => {
  const f = fixture(t);
  configured(f);
...
test('external success requires explicit status and rejects secrets, foreign files and conflicting findings', (t) => {
  const f = fixture(t, 'export const valid = 1;');
  configured(f);
  probe(f);
  const admitted = ticket(f),
...
test('results reject outdated snapshots, connections, tools and late timestamps', (t) => {
  const f = fixture(t);
  configured(f);
  probe(f);
  const admitted = ticket(f),
  ...
  const next = ticket(f);
  f.deliver('export const changed = 2;');
  assert.throws(
...
test('application connectors prepare a revision-bound integration without sending, provisioning or jobs', (t) => {
  const f = fixture(t);
...
test('probe and result text reject credential-bearing URLs without persisting their values', (t) => {
  const f = fixture(t);
  configured(f);
...
test('changed connector configuration makes its retained quality evidence reevaluable', (t) => {
  const f = fixture(t, 'export const valid = 1;');
  configured(f);
  probe(f);
  const admitted = ticket(f);
...
test('expired execution tickets reject new results without producing quality evidence', (t) => {
...
test('changed source bytes reject an external receipt even when its supplied fingerprint matches the ticket', (t) => {
  const f = fixture(t);
  configured(f);
  probe(f);
  const admitted = ticket(f);
...
test('MCP returns must name an observed tool and keep blocked browser findings distinct from success', (t) => {
  const f = fixture(t);
  configured(f, 'playwright-mcp');
  probe(f, {
    tool: { name: 'Playwright MCP', version: 'fixture' },
...
test('oversized receipts and corrupt connector registers fail without overwriting persisted data', (t) => {
  const f = fixture(t);
  configured(f);
  probe(f);
  const admitted = ticket(f);
...
test('MCP interface changes invalidate dependent evidence while equivalent probes preserve it', (t) => {
  const f = fixture(t);
  configured(f, 'playwright-mcp');
  const fields = {
    tool: { name: 'Playwright MCP', version: 'fixture' },
...
test('a selected inactive revision can be checked without validating the applied revision', (t) => {
  const f = fixture(t, 'export const selected = 1;');
  configured(f);
  probe(f);
  const active = f.deliver('export const applied = 2;');
  const admitted = ticket(f);
  ...
  f.deliver('export const changedAgain = 3;');
  assert.throws(
...
function businessFixture(t) {
  const f = fixture(t, 'export const valid = true;');
...
test('business tickets capture exact criteria and reject a changed brief without claiming success', (t) => {
...
test('changed business criteria invalidate only dependent evidence and preserve the recorded objective', (t) => {
  const f = businessFixture(t);
  const admitted = ticket(f, { checkId: 'business-journey' });
...
test('business checks require explicit bounded criteria and old unbound records need reevaluation', (t) => {
  const f = businessFixture(t);
  const admitted = ticket(f, { checkId: 'business-journey' });
  const input = result(admitted, { source: { kind: 'host-api' } });
  importExternalQualityResult(f.store, input);
