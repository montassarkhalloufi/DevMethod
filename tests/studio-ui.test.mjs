...
async function fixture(t, customize = () => {}, hash = '', options = {}) {
  ...
  // Historical behavior assertions deliberately exercise the retained French UI.
...
test('English is the initial UI and switching languages preserves drafts, focus, preview and project state', async (t) => {
  ...
  assert.equal(f.el('tab-checks').textContent.trim(), 'Checks');
  f.input('idea', 'Mon idée reste exactement comme saisie');
  f.input('request', 'Conserver ce brouillon français');
  f.el('request').focus();
  const before = structuredClone(f.state());
  const frame = f.el('preview').contentWindow;
  const language = document.querySelector('[data-studio-language]');
  for (const [locale, label] of [
    ['fr', 'Vérifications'],
    ['en', 'Checks'],
  ]) {
    language.value = locale;
    language.dispatchEvent(new f.dom.window.Event('change', { bubbles: true }));
    assert.equal(document.documentElement.lang, locale);
    assert.equal(f.el('tab-checks').textContent.trim(), label);
    assert.equal(
      f.el('draft-status').textContent,
      locale === 'fr' ? 'Brouillon modifié…' : 'Draft changed…',
    );
    assert.equal(
      f.el('project-save-status').textContent,
      locale === 'fr' ? 'Modifications non enregistrées.' : 'Unsaved changes.',
    );
    assert.equal(f.el('idea').value, 'Mon idée reste exactement comme saisie');
    assert.equal(f.el('request').value, 'Conserver ce brouillon français');
    assert.equal(document.activeElement.id, 'request');
...
test('project technology choice survives polling and is submitted explicitly', async (t) => {
  const f = await fixture(t);
  assert.equal(f.el('project-expected-profile').value, '');
  f.input('project-expected-profile', 'react-ts');
  f.state().version++;
  await f.app.refresh();
  assert.equal(f.el('project-expected-profile').value, 'react-ts');
  await f.submit('project-form');
  assert.equal(f.calls.find((call) => call.route === 'project').input.expectedProfile, 'react-ts');
  await f.app.refresh();
  assert.equal(f.el('project-expected-profile').value, 'react-ts');
});

test('history adoption sends the reviewed version and requires a fresh confirmation after concurrent changes', async (t) => {
  const f = await fixture(
    t,
    (state) => {
      state.revisions = [revision('v1'), revision('v2')];
      state.activeRevision = 'v1';
    },
    '#history',
  );
  ...
  f.api.activationReview = async (id) => ({
    ...
    revision: { id, title: id },
    reviewKey: 'review-' + f.state().version,
    canActivate: true,
    admission: { allowed: true, reason: 'Controlled test only' },
    control: null,
  });
  const original = f.api.change;
  const versions = [];
  f.api.change = async (route, version, input) => {
    versions.push(version);
    if (version !== f.state().version)
      throw Object.assign(new Error('Review changed'), { status: 409 });
    if (route === 'activate') f.state().activeRevision = input.id;
    return original(route, version, input);
  };
  f.el('activate-v2').click();
  await setImmediate();
  const dialog = f.dom.window.document.querySelector('.activation-review-dialog');
  assert.equal(dialog.open, true);
  assert.equal(f.calls.length, 0);
  const reason = dialog.querySelector('textarea');
  reason.value = 'Controlled adoption test';
  f.state().version++;
  await f.app.refresh();
  const submit = () =>
    dialog
  ...
  assert.equal(f.state().activeRevision, 'v1');
  assert.equal(reason.value, 'Controlled adoption test');
  [...dialog.querySelectorAll('button')]
    .find((button) => button.textContent === 'Actualiser l’examen')
    .click();
  await setImmediate();
  assert.deepEqual(versions, [1]);
  submit();
  ...
  assert.equal(f.calls.at(-1).input.reviewKey, 'review-2');
  assert.equal(f.state().activeRevision, 'v2');
...
test('comparison labels, source and iframe follow the candidate identity on the read-only origin', async (t) => {
  ...
  const language = f.dom.window.document.querySelector('[data-studio-language]');
  for (const [locale, prefix] of [
    ...
    language.value = locale;
    language.dispatchEvent(new f.dom.window.Event('change', { bubbles: true }));
    assert.equal(f.el('preview-scenario').textContent, prefix + ' · Formulaire');
    assert.equal(f.el('preview-scenario').value, 'form');
  }
  f.el('tab-code').click();
  await setImmediate();
...
test('element targeting accepts only the preview origin and the actual iframe window', async (t) => {
  ...
  language.value = 'en';
  language.dispatchEvent(new f.dom.window.Event('change', { bubbles: true }));
  assert.equal(f.el('element-label').textContent, 'Selected element: Mes lectures');
  assert.equal(f.el('element-selection').hidden, false);
...
test('evidence dock excludes checks from other revisions and exposes interrupted work and unknown costs', (t) => {
  ...
  assert.match(root.textContent, /0 contrôle\(s\) de livraison passé\(s\) · 1 échoué/);
...
test('a runtime-only control change refreshes checks and status without attributing it to the displayed revision', async (t) => {
  const f = await fixture(t, (state) => {
    state.revisions = [revision('active')];
    state.activeRevision = 'active';
  });
  const graph = { revisionId: 'candidate', nodes: [], edges: [] };
  f.runtime.control = {
    graph,
  ...
  await f.app.refresh();
  assert.match(f.el('checks-list').textContent, /La consommation est inconnue/);
  assert.match(f.el('checks-list').textContent, /ne concerne pas la version affichée \(active\)/);
  assert.match(
    f.el('evidence-dock').textContent,
    /Exécution arrêtée · version évaluée : candidate/,
  );
  f.runtime.control = {
    ...f.runtime.control,
    autonomy: {
      ...f.runtime.control.autonomy,
  ...
  assert.doesNotMatch(f.el('checks-list').textContent, /La consommation est inconnue/);
  assert.match(f.el('evidence-dock').textContent, /Vérifications à renforcer/);
});
