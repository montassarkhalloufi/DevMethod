...
const checkpoint = {
  format: 1,
  scope: 'AC-TITLE unit/HTTP/model verification only. AC-STORE and AC-WEB remain separate checks.',
  status: 'active',
  ...
  evidence: [
    {
      id: 'unit',
      path: 'evidence/unit.log',
      sha256: digest(log),
      sourceIds: before.sources.map((s) => s.id),
      dependsOn: [],
      outcome,
      criterionIds: ['AC-TITLE'],
      kind: 'automated',
      revision: `${before.git.commit}; working-content ${before.git.diffSha256}`,
    },
  ],
};
  ...
  JSON.stringify(
    {
      ...
      command: 'npm test',
      outcome,
      checkpoint: 'examples/fullstack/evidence/checkpoint.json',
      report,
      limitation:
        'This invocation checks unit/HTTP/model only. Database, production e2e and native host behavior are separate evidence.',
    },
    null,
    2,
  ),
