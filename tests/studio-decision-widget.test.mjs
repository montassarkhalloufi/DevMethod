...
async function fixture(t, options = {}) {
  const dom = new JSDOM(
    '<html lang="fr" data-studio-language-ready><main id="root"></main></html>',
    {
      url: 'http://localhost',
...
test('comparison binds checks to the exact revision and leaves image simulations untested', () => {
  ...
  assert.match(
    describeComparison(state, item, 'proposal', 'fr').label,
  ...
  const after = describeComparison(state, item, 'proposal', 'fr');
  assert.match(after.label, /non appliquée.*Aucun contrôle/);
  assert.equal(describeComparison(state, item, 'before', 'fr').revisionId, 'current');
  assert.equal(describeComparison(state, item, 'before', 'fr').element.selector, '#booking');
  assert.equal(
  ...
  assert.throws(
    () => comparisonURL('http://127.0.0.1:4331', 'candidate', '//example.com', 'fr'),
    /invalide/,
  );
});
...
test('optional explanation produces an explicit neutral action trace rather than a rejected empty reason', async (t) => {
  const dom = new JSDOM(
    '<html lang="fr" data-studio-language-ready><aside id="discussion"><details id="activity"></details><div id="active-decision"></div></aside>',
  );
  let props;
  const calls = [];
