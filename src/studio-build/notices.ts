...
function bundledRoots(context: Resolution, metafile: Metafile): Set<string> {
  const roots = new Set<string>();
  for (const output of Object.values(metafile.outputs)) {
    for (const [input, contribution] of Object.entries(output.inputs)) {
      if (!input.startsWith('trusted:') || contribution.bytesInOutput === 0) continue;
      const file = input.slice('trusted:'.length);
      const owner = context.roots
        .filter((root) => inside(root, file))
        .sort((left, right) => right.length - left.length)[0];
      if (!owner) throw new Error('Provenance de bibliothèque compilée introuvable.');
      roots.add(owner);
    }
  }
  return roots;
...
function packageNotice(context: Resolution, root: string): { name: string; text: string } {
  const metadata = JSON.parse(context.read(path.join(root, 'package.json')) ?? 'null') as {
    name?: unknown;
    version?: unknown;
  } | null;
...
export function compiledNotices(context: Resolution, metafile: Metafile, hasCSS: boolean): string {
  const roots = bundledRoots(context, metafile);
  // Styles go through Tailwind before esbuild, so it is absent from the JS metafile.
  if (hasCSS) roots.add(packageRoot('tailwindcss'));
  const notices = [...roots]
  ...
  for (const name of ['THIRD_PARTY_NOTICES.md', 'THIRD_PARTY_NOTICES.txt']) {
    ...
    if (content) sections.push(`Project notice: ${name}\n${content.toString('utf8')}`);
  }
