...
export function inspectClosure(root, missionPath, checkpointPath) {
    ...
    if (mission.acceptance.some(c => c.changes.some(secretPath)))
    ...
    if (checkpoint.status === 'invalid')
        return { format: 1, missionId: mission.id, status: 'invalid', findings: checkpoint.findings, criteria: [], limitations };
    const state = input;
    const findings = [];
    ...
    for (const source of mission.sources) {
        ...
        if (!pin || pin.id !== source.id)
            findings.push({ code: 'context-not-pinned', message: `Mission source ${source.id} must use the same ID and path in checkpoint sources.` });
    }
    if (!state.git)
        findings.push({ code: 'revision-missing', message: 'Closure requires a Git-aware checkpoint; legacy resume remains supported.' });
    const knownCriteria = new Set(mission.acceptance.map(c => c.id));
    for (const e of state.evidence)
        if (e.criterionIds?.some(id => !knownCriteria.has(id)))
