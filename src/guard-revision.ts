...
function entries(output: string, pattern: RegExp): Map<string, GitEntry> {
  const records = output.split('\0').filter(Boolean);
  if (records.length > MAX_FILES) throw new Error('Committed context exceeds the file limit.');
  const result = new Map<string, GitEntry>();
  for (const record of records) {
    const match = pattern.exec(record);
    if (!match) throw new Error('Unsupported Git entry.');
    const mode = match[1]!;
    const oid = match[2]!;
    const file = match[3]!;
    if (
...
function matchingIndex(root: string, committed: Map<string, GitEntry>): boolean {
  const indexed = entries(
    gitOutput(root, ['ls-files', '--stage', '-z']),
    /^(\d{6}) ([a-f0-9]{40}(?:[a-f0-9]{24})?) 0\t(.+)$/,
  );
  return (
    indexed.size === committed.size &&
...
function evidencePath(file: string, snapshot: GuardSnapshot): boolean {
  return (
    file === snapshot.reportPath ||
    file === snapshot.artifactPath ||
    file.startsWith(`${snapshot.artifactPath}/`)
  );
}

function declaredInputsCommitted(
  root: string,
  snapshot: GuardSnapshot,
  committed: Map<string, GitEntry>,
): boolean {
  if (!safePath(snapshot.reportPath) || !safePath(snapshot.artifactPath)) return false;
  // Evidence cannot disguise tracked code or configuration as an uncommitted observation.
  if ([...committed.keys()].some((file) => evidencePath(file, snapshot))) return false;
...
function committedBytesMatch(
  ...
  for (const entry of committed.values()) {
    ...
    total += info.size;
    if (!info.isFile() || info.size > MAX_FILE_BYTES || total > MAX_TOTAL_BYTES) return false;
...
export function hasCommittedGuardInputs(root: string, snapshot: GuardSnapshot): boolean {
  try {
    ...
    const committed = entries(
      gitOutput(root, ['ls-tree', '-r', '-z', '--full-tree', revision]),
      /^(\d{6}) blob ([a-f0-9]{40}(?:[a-f0-9]{24})?)\t(.+)$/,
    );
    return (
      matchingIndex(root, committed) &&
