...
export function validateMission(v) {
    ...
    if (!Array.isArray(v.acceptance) ||
        !v.acceptance.length ||
        v.acceptance.length > 128 ||
        !v.acceptance.every((c) => object(c) &&
            id(c.id) &&
            text(c.description) &&
            list(c.changes) &&
            c.changes.length > 0 &&
    ...
    if (!Array.isArray(v.dependencies) ||
        ...
        !v.dependencies.every((d) => object(d) &&
            id(d.id) &&
            text(d.owner) &&
            text(d.detail) &&
    ...
    if (sourceIds.size !== mission.sources.length ||
    ...
    if (!Array.isArray(v.contradictions) ||
        v.contradictions.length > 128 ||
        !v.contradictions.every((c) => object(c) &&
            list(c.sourceIds) &&
...
function sourceHash(root, file) {
    ...
    if (body.includes('\0') ||
        /-----BEGIN [A-Z ]*PRIVATE KEY-----|(?:api[_-]?key|password|secret|token)\s*[:=]\s*["']?[A-Za-z0-9_\-/+]{20,}/i.test(body))
...
export function inspectContext(root, input) {
    ...
    return {
        ...
        status: missionStatus(mission) === 'blocked'
            ? 'blocked'
            : stale
                ? 'reverify'
