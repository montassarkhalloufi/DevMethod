...
2 invalid invocation. File integrity does not prove native agent behavior.
`;
...
try {
  ...
  if (values.help) console.log(help);
    ...
    if (positionals.length !== 1 || !['init', 'doctor', 'update-preview'].includes(positionals[0] ?? '')) throw new Error(help);
    if (positionals[0] === 'update-preview') {
      ...
      if (values.json !== undefined) throw new Error('--json is supported only by doctor and update-preview');
