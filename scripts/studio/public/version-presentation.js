...
function displayedTarget(state, options, locale = getLocale()) {
  ...
  const comparison = describeComparison(
    state,
    compared,
    options.comparisonSide ?? 'proposal',
    locale,
  );
  ...
  return revision
    ...
    : {
        kind: 'empty',
        simulation: false,
        label: t('Aucune version affichée', 'No version displayed'),
      };
}
...
/** @returns {Displayed} */
function displayedVersion(state, target, proposal, before, locale = getLocale()) {
  const t = (fr, en, values) => translate(fr, en, values, locale);
  ...
  if (target.kind === 'image')
    return {
      ...
      label: t(
        'Simulation visuelle · aucune version exécutée',
        'Visual simulation · no version executed',
      ),
    };
  if (target.kind !== 'revision')
    return {
  ...
  const labels = {
    applied: t('Version appliquée', 'Applied version'),
    proposal: t('Proposition non appliquée', 'Unapplied proposal'),
    local: before
      ? t('Version de départ non appliquée', 'Unapplied starting version')
      : t('Version locale non appliquée', 'Unapplied local version'),
  };
  return {
    kind: 'revision',
    status,
    ...
    label: `${revision.origin?.kind === 'import' ? t('Référence importée', 'Imported reference') : labels[status]} · ${revision.title}`,
  };
}

/** @returns {CheckPresentation} */
function checksForDisplay(state, displayed, locale = getLocale()) {
  const t = (fr, en, values) => translate(fr, en, values, locale);
  const items = displayed.revisionId
    ? state.checks
        ...
        .map((entry) => ({ ...entry }))
    : [];
  const delivery = items.filter((entry) => entry.kind === 'command');
  const passed = delivery.filter((entry) => entry.status === 'passed').length;
  const failed = delivery.filter((entry) => entry.status === 'failed').length;
  const observations = items.filter((entry) => entry.kind !== 'command').length;
  /** @type {CheckPresentation['status']} */
  let status = 'not-applicable';
  let label = t(
    'Aucun contrôle de livraison attribuable à cet aperçu',
    'No delivery checks attributable to this preview',
  );
  if (displayed.revisionId) {
    status = failed ? 'failed' : delivery.length ? 'passed' : 'unverified';
    label = items.length
      ? t(
          '{value0} contrôle(s) de livraison passé(s), {value1} échec(s) sur {value2} · couverture à établir',
          '{value0} delivery check(s) passed, {value1} failure(s) on {value2} · coverage to establish',
          {
            value0: passed.toLocaleString(locale),
            value1: failed.toLocaleString(locale),
            value2: displayed.revisionId.slice(0, 8),
          },
        )
      : t(
          'Aucun contrôle de livraison enregistré sur {value0}',
          'No delivery checks recorded on {value0}',
          { value0: displayed.revisionId.slice(0, 8) },
        );
  }
  if (observations)
    label += t(
      ' · {value0} observation(s) non attestée(s)',
      ' · {value0} unattested observation(s)',
      { value0: observations.toLocaleString(locale) },
    );
  return {
...
function acceptedVisualLabel(approval, locale = getLocale()) {
  const t = (fr, en, values) => translate(fr, en, values, locale);
  const subject = approval.referenceId
    ? t('Maquette acceptée', 'Mockup accepted')
    : approval.route || approval.element
      ? t('Rendu ciblé accepté', 'Targeted visuals accepted')
      : t('Rendu accepté pour cette version', 'Visuals accepted for this version');
  return `${subject}${approval.source === 'agent' ? t(' · accord de l’agent par délégation', ' · agent approval under delegation') : t(' · accord de votre part', ' · approved by you')}`;
}

/** @returns {VisualPresentation} */
function visualForDisplay(proposals, target, responsibility, locale = getLocale()) {
  const t = (fr, en, values) => translate(fr, en, values, locale);
  ...
  let label = t(
    'Aucun accord visuel enregistré sur cet aperçu',
    'No visual approval recorded for this preview',
  );
  if (approval) {
    status =
    ...
    label =
      status === 'accepted'
        ? acceptedVisualLabel(approval, locale)
        : t(
            'Accord antérieur de l’agent · validation visuelle à confirmer par vous',
            'Previous agent approval · visual approval now requires your confirmation',
          );
  }
  if (pending) {
    status = 'pending';
    label = t(
      'Proposition visuelle à examiner · accord encore attendu',
      'Visual proposal to review · approval still pending',
    );
  }
  return {
    responsibility,
    responsibilityLabel: responsibility === 'user' ? t('Vous', 'You') : 'Agent',
    status,
    label,
    approval,
  };
 ...
 */
export function presentation(state, options = {}, locale = getLocale()) {
  const t = (fr, en, values) => translate(fr, en, values, locale);
  const target = displayedTarget(state, options, locale);
  ...
  const displayed = displayedVersion(state, target, related, before, locale);
  ...
  return {
    ...
    active: active
      ? {
          ...
          title: active.title,
          label: `${active.origin?.kind === 'import' ? t('Référence importée', 'Imported reference') : t('Version appliquée', 'Applied version')} · ${active.title}`,
        }
      : null,
    proposal: related
      ? {
      ...
      : null,
    checks: checksForDisplay(state, displayed, locale),
    visual: visualForDisplay(proposals, target, responsibility, locale),
  };
}
