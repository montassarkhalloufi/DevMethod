      ...
      function Harness() {
        ...
        return <>
          ...
          {definition ? <ConnectorGuide definition={definition} draft={draft} preparation={request.preparation}
            preparing={request.loading} error={request.error} step={options.step} onChange={value=>{request.reset();setDraft(value);}}
...
function fixture(t, fetcher, options = {}) {
  ...
  dom.window.document.documentElement.setAttribute('data-studio-language-ready', '');
...
test('guided Slack choices are progressive, keep keyboard focus and render only the server preparation before explicit apply', async (t) => {
  ...
  const f = fixture(t, async (url, init) => {
    ...
    return reply(readConnectorGuides('fr'));
  });
  ...
  assert.equal(posts[0].url, '/api/connectors/guides/prepare?language=fr');
...
test('reopening the saved verification step preserves answers without claiming they changed', async (t) => {
  const posts = [];
  const f = fixture(
    t,
    async (_url, init) => {
      if (!init?.method) return reply(readConnectorGuides('fr'));
      posts.push(JSON.parse(init.body));
      return reply(prepareConnectorGuide(posts.at(-1)));
    },
    { input: input(), step: 2 },
  );
  await until(() => button(f, 'Vérifier la préparation'));
  assert.doesNotMatch(f.document.body.textContent, /Vos réponses ont changé/);
  assert.match(f.document.body.textContent, /Vos réponses sont conservées/);
  assert.equal(posts.length, 0);
  button(f, 'Vérifier la préparation').click();
  await until(() => f.document.querySelector('.connector-guide-summary'));
  assert.deepEqual(posts, [input()]);
  assert.match(f.document.querySelector('.connector-guide-summary').textContent, /chat:write/);
  assert.equal(f.applied.length, 0);
});
...
test('stale preparation responses, reset and disposal cannot restore previous provider state', async (t) => {
  ...
  const f = fixture(t, (url, init) =>
    init?.method === 'POST'
      ...
      : Promise.resolve(reply(readConnectorGuides('fr'))),
  );
...
test('unreadable catalog and mismatched preparation remain explicit and retryable', async (t) => {
  ...
  const f = fixture(
    ...
    async (_url, init) => {
      ...
      return reply(
        ++reads === 1 ? { guides: [{ optionId: 'broken' }] } : readConnectorGuides('fr'),
      );
    },
    { input: input() },
  );
...
test('response readers refuse executable source links, false connected states and malformed answer types', async (t) => {
  const f = fixture(t, async () => reply(readConnectorGuides('fr')));
  const api = f.dom.window.GuideTest;
  const definitions = readConnectorGuides('fr');
