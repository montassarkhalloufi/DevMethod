...
type DetailSection = 'evidence' | 'correction' | 'references';
...
interface DetailActions {
  back(): void;
  selectSection(section: DetailSection): void;
}
...
export function renderFindingDetail(
  ...
  const back = button('← Retour aux résultats', 'back-button', () => {
    actions.back();
    (
      document.querySelector<HTMLButtonElement>('.finding.selected') ||
      document.getElementById('view-panel-findings')
    )?.focus();
  });
  parent.append(back);
  ...
  parent.append(
    tabs(
      ...
      state.section,
      (id) => {
        actions.selectSection(id as DetailSection);
      },
      'Détail du constat',
      'detail-content',
    ),
...
function renderDetailHeader(
  ...
  const filterNote = el(
    'p',
    'notice',
    'Ce constat est hors de la sélection filtrée. Son détail reste disponible.',
  );
  ...
  badges.append(
    ...
    badge(resolutionLabels[f.resolution], 'neutral', '◌'),
  );
  parent.append(
    badges,
    el('p', 'impact', f.impact),
    el(
      'p',
      'location',
      `${f.location.path}${f.location.line ? `:${f.location.line}` : ''}${f.location.component ? ` · ${f.location.component}` : ''}`,
    ),
  );
}
...
function renderDetailSection(
  ...
  if (section === 'evidence') evidenceDetail(review, f, content);
  if (section === 'correction') {
    ...
    textBlock(
      'État de résolution',
      `${resolutionLabels[f.resolution]} · Preuves de résolution : ${f.resolutionEvidenceIds.join(', ') || 'aucune'}`,
      content,
    );
...
function renderDetailActions(f: ReviewFinding, parent: HTMLElement, actions: DetailActions): void {
  const footer = el('footer', 'detail-actions');
  footer.append(
    button('↗ Lien vers ce constat', 'button secondary', () => {
  ...
  if (f.evidenceIds.length)
    footer.append(
      button('▤ Voir la preuve', 'button primary', () => {
