...
export function createTask(title, note, id) {
  if (typeof title !== 'string' || !title.trim())
    throw new Error('Donnez un titre à votre priorité.');
  if (title.trim().length > TITLE_LIMIT) throw new Error('Le titre est limité à 160 caractères.');
  if (typeof note !== 'string' || note.trim().length > NOTE_LIMIT)
    throw new Error('La note est limitée à 500 caractères.');
...
export function removeTask(tasks, id) {
  ...
  return {
    ...
    removed: index < 0 ? null : { task: tasks[index], index },
  };
}
export function restoreTask(tasks, removed) {
  if (!removed || tasks.some((task) => task.id === removed.task.id)) return tasks;
  const result = [...tasks];
  result.splice(Math.min(removed.index, result.length), 0, removed.task);
  return result;
}
export function filterTasks(tasks, filter) {
  return tasks.filter((task) =>
    filter === 'done' ? task.done : filter === 'active' ? !task.done : true,
  );
}
export function decodeTasks(raw) {
  ...
  for (const task of data.tasks) {
    if (
      ...
      !task.title.trim() ||
      task.title.length > TITLE_LIMIT ||
      typeof task.note !== 'string' ||
      task.note.length > NOTE_LIMIT ||
      typeof task.done !== 'boolean'
    )
      throw new Error('Données enregistrées illisibles.');
    ids.add(task.id);
  }
