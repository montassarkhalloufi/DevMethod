...
function fixture(t) {
  ...
  const { revision } = jobs.finish({ jobId: claim.job.id, title: 'Notes' });
  fs.writeFileSync(
...
test('editor saves exact text durably, rejects stale CAS and unsafe paths without losing changes', (t) => {
  const f = fixture(t),
...
test('manual adoption uses exact build and new check, reopens at adopted contents, and respects visual reservation', async (t) => {
  const f = fixture(t),
    first = f.editor.read(f.revision.id);
  const saved = f.editor.save({
    ...first,
  ...
  const applied = f.editor.apply({ ...built, title: 'Edited' });
  assert.equal(applied.state.activeRevision, applied.revision.id);
  assert.equal(applied.draft.baseRevision, applied.revision.id);
  assert.equal(
    applied.draft.files.find((x) => x.path === 'app.js').content,
    'globalThis.example = 2;',
  );
  assert.equal(applied.state.checks.filter((x) => x.revisionId === applied.revision.id).length, 1);
  assert.deepEqual(applied.draft.changedPaths, []);
...
test('changed base and substituted source symlink are refused while the durable draft remains intact', async (t) => {
  ...
  f.jobs.finish({ jobId: claim.job.id, title: 'Other' });
  assert.throws(() => f.editor.save({ ...saved, changes: [] }), { status: 409 });
...
test('editor-only runtime observer precedes application scripts and reports bounded errors and rejections to Studio', async (t) => {
  const f = fixture(t);
  f.store.close();
  ...
  assert.ok(
    !(await (await fetch(runtime.previewOrigin)).text()).includes('devmethod-runtime-error'),
  );
});
...
test('ES module import followed by a trailing syntax error fails both editor and revision verification', async (t) => {
  ...
  const { revision } = f.jobs.finish({ jobId: claim.job.id, title: 'Invalid syntax fixture' });
