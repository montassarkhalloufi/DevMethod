...
import { jobSourceRevision, nextQueuedJob } from './candidate-request-record.mjs';
import { assertCandidateRequestCurrent } from './candidate-request.mjs';
export { contextKey } from './job-context.mjs';

function browserVerificationContext(store) {
  let configuration;
  try {
    configuration = readBrowserConfiguration(store);
  } catch {
    return {
      enabled: false,
      reason: 'Configuration navigateur illisible ; aucun lancement autorisé.',
    };
  }
  return {
    enabled: configuration.enabled,
    automatic: configuration.automatic,
    manifest: 'devmethod.browser.json',
    protocol: 'studio-browser-v1',
    documentation: fileURLToPath(new URL('../../docs/STUDIO-BROWSER-CHECKS.md', import.meta.url)),
    instructions:
      'When enabled, include bounded observable scenarios for the actual brief criteria. Read the protocol documentation; use accessible targets or test IDs, synthetic inputs and {{nonce}}. The Studio check runs a separate browser and empty isolated data. Do not launch a browser or install dependencies in the job, invent successful assertions, change browser configuration, or claim semantic coverage from declared criterionIds. When automatic is explicitly enabled, Studio may run the check after admitting your candidate; otherwise it is launched from Verifications. Submitting the manifest does not itself execute it.',
  };
}

function prepareRevision(before, job, input, files, compilation) {
  const base = before.revisions.find((r) => r.id === job.baseRevision);
  const sourceBase = before.revisions.find((r) => r.id === jobSourceRevision(job));
  const sourceOnly = Boolean(before.import && sourceBase?.profile === 'source-only');
  const planning = !domain.hasApprovedPlan(before);
  const comparison = job.candidateRequest ? sourceBase : base;
  const changed = JSON.stringify(files) !== JSON.stringify(comparison?.files ?? []);
...
export function createJobs(store, { mcpContext } = {}) {
  ...
  function claim(worker) {
    ...
    const next = nextQueuedJob(before);
    if (next) assertCandidateRequestCurrent(store, before, next);
    let job;
    ...
    try {
      ...
      const base = state.revisions.find((r) => r.id === jobSourceRevision(job));
      if (base)
    ...
    const context = {
      ...
      request: job.request,
      verification: {
        browser: browserVerificationContext(store),
        command: [
          process.execPath,
          fileURLToPath(new URL('./check-work.mjs', import.meta.url)),
          '--source',
          workDirectory,
        ],
        scope:
          'JSON, scripts HTML intégrés, syntaxe JavaScript et compilation React strict applicables. Aucun script package exécuté. Ce résultat local est une observation de l’agent ; Studio réexécute ses contrôles avant admission. Le comportement reste à vérifier séparément.',
      },
      element: job.element,
      ...(job.candidateRequest ? { candidateRequest: structuredClone(job.candidateRequest) } : {}),
  ...
  function finalize(input, compilation = null, expectedFiles = null, checks = [], options = {}) {
    const before = store.read(),
    ...
    if (expectedFiles && JSON.stringify(files) !== JSON.stringify(expectedFiles))
      throw new Error('Le code a changé pendant la vérification ; résultat non adopté.');
    ...
    if (
      ...
      revision.profile = 'react-ts';
    // Validate the whole transition before creating the immutable snapshot.
    domain.finishJob(
      structuredClone(before),
      { ...input, revision },
      { checks: revision ? checks : [], deferActivation: options.deferActivation === true },
    );
    let target;
    try {
      if (revision) {
  ...
  async function finishReact(input, source, files, documentChecks, options) {
  ...
  function finish(input, options = {}) {
    ...
    const source = safeFile(store.root, `work/${job.id}/app`),
      files = fileManifest(source);
    const base = before.revisions.find((revision) => revision.id === jobSourceRevision(job));
    const comparison = job.candidateRequest
      ? base
      : before.revisions.find((revision) => revision.id === job.baseRevision);
    const changed = JSON.stringify(files) !== JSON.stringify(comparison?.files ?? []);
    if (changed && base?.profile !== 'source-only' && before.project.expectedProfile) {
      const actual = sourceProfile(source, files);
      if (actual !== before.project.expectedProfile)
        throw Object.assign(
          new Error(
            `Stack attendue : ${before.project.expectedProfile} ; livraison détectée : ${actual}. Travail conservé, sans adoption.`,
          ),
          { status: 409 },
        );
    }
    const documents =
      changed && base?.profile !== 'source-only' ? candidateDocumentChecks(source, files) : [];
    if (
      ...
      return finishReact(input, source, files, documents, options);
    if (
      changed &&
      base?.profile !== 'source-only' &&
      files.some((file) => /\.(?:m?js|cjs)$/.test(file.path))
    )
      return Promise.all([documents, candidateSyntaxChecks(source, files)]).then(
        ([documentChecks, checks]) =>
          finalize(input, null, files, [...documentChecks, ...checks], options),
      );
    if (documents instanceof Promise)
      return documents.then((checks) => finalize(input, null, files, checks, options));
    return finalize(input, null, files, documents, options);
  }
