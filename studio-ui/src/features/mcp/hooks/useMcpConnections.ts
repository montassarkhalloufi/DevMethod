...
function openAuthorization() {
  ...
  popup.opener = null;
  popup.document.title = 'Connexion du serveur MCP';
  popup.document.body.textContent = 'Préparation de votre connexion sécurisée…';
  return popup;
}
...
export function useMcpConnections(
  ...
  async function connect(input: McpConnectInput) {
    ...
    try {
      if ((input.auth || 'oauth') === 'oauth') popup.current = openAuthorization();
