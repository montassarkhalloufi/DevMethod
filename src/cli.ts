...
function studioEntry(): string | null {
  const source = new URL('../scripts/studio/cli.mjs', import.meta.url);
  if (existsSync(source)) return source.href;
  try {
    return import.meta.resolve('devmethod-studio/cli');
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ERR_MODULE_NOT_FOUND') throw error;
    console.error(
      'Studio is a separate package. Install devmethod-studio explicitly, then run devmethod-studio. Nothing was downloaded.',
    );
    process.exitCode = 2;
    return null;
  }
