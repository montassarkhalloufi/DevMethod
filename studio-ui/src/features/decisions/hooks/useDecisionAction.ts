...
function useReasonDraft(identity: DraftIdentity) {
  ...
  return {
    ...
    storageWarning: storageWarning
      ? t(
          storageUnavailable,
          'Local storage is unavailable. Your reason remains in memory; copy it before leaving.',
        )
      : '',
    current,
    clearApproved,
...
export function useDecisionAction(identity: DraftIdentity) {
  ...
  const error =
    failureMessage === null
      ? ''
      : failureMessage || t('Enregistrement impossible. Réessayez.', 'Unable to save. Try again.');
  ...
  return {
    status,
    error: translateStudioError(error, locale),
    run,
    reason: draft.reason,
