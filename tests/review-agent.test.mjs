...
for (const host of Object.keys(tools))
  for (const type of ['commonjs', 'module', 'absent']) {
    test(`installed report flow: ${host}, ${type}, no npm or source checkout`, (t) => {
      ...
      record.id = 'INSTALLED-FIXTURE';
      record.title = 'Installed runtime test';
...
cp.spawnSync = (command, args, options) => { fs.writeFileSync('opened.json', JSON.stringify({command,args,options})); return {status:process.env.REVIEW_TEST_OPEN_FAILURE ? 1 : 0}; }; syncBuiltinESMExports();`,
      ...
      const run = (args, failOpen = false) =>
        spawnSync(
          process.execPath,
          ['--import', pathToFileURL(path.join(root, 'open-probe.mjs')).href, script, ...args],
          {
            cwd: root,
            encoding: 'utf8',
            env: { ...process.env, PATH: '', REVIEW_TEST_OPEN_FAILURE: failOpen ? '1' : '' },
          },
        );
      const args = [
        '--review',
      ...
      assert.equal(
        run(['--review', 'invalid.json', '--output', 'invalid.html', '--markdown', 'invalid.md'])
          .status,
        2,
      );
