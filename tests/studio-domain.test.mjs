...
test('guided completion stays ready until activation, checks remain tied to old revision', () => {
  ...
  assert.equal(state.checks.filter((check) => check.revisionId === state.activeRevision).length, 0);
  validateStudioState(state);
});

test('delegated completion activates real revision but creates no successful check', () => {
  const state = createInitialStudioState();
  updateProject(state, {
    name: 'Les Ateliers',
