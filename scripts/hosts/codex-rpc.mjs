...
export function startCodexRpc({
  ...
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 120000)
    throw new Error('Invalid RPC lifetime');
  ...
  function send(message) {
    if (stopped) throw new Error('RPC closed');
  ...
  function handleMessage(message) {
    if ('id' in message && !message.method) {
      ...
      send({
        id: message.id,
        error: { code: -32601, message: 'Client requests disabled in probe' },
      });
  ...
  child.stdout.on('data', (chunk) => {
    ...
    if (bytes > 2 * 1024 * 1024) return close(new Error('RPC output limit'));
    buffer += chunk;
    let end;
  ...
  child.stderr.on('data', (chunk) => {
    bytes += chunk.length;
    if (bytes > 2 * 1024 * 1024) close(new Error('RPC output limit'));
  });
