...
for (const tool of Object.keys(tools))
  test(`doctor accepts full and subset ${tool} installations without changes`, (t) => {
...
test('customized templates and skills are warnings, preserved byte for byte', (t) => {
  ...
  for (const name of names)
    fs.appendFileSync(path.join(destination, name), '\nProject customization\n');
  const before = snapshot(destination);
  const report = diagnose(destination);
...
test('doctor rejects malformed, incomplete and unsupported manifests before reading payload', (t) => {
  const edits = [
    ...
    (m) => {
      m.skills.push('unknown');
    },
    (m) => {
      m.skills.push('project-foundation');
    },
    (m) => {
      m.skills = ['scoped-delivery'];
    },
    (m) => {
      delete m.files['DEVMETHOD-LICENSE'];
    },
    (m) => {
      delete m.files['.agents/skills/project-foundation/SKILL.md'];
    },
    (m) => {
      m.files['PROJECT_PROFILE.md'] = 'not-a-hash';
    },
    (m) => {
      m.files = [];
    },
...
test('untrusted manifest cannot select traversal, absolute, foreign-host or arbitrary project paths', (t) => {
  for (const name of [
    ...
    '.claude/skills/project-foundation/SKILL.md',
    '.agents/skills/unselected/SKILL.md',
  ]) {
    const { destination } = fixture(t);
    editManifest(destination, (m) => {
      m.files[name] = '0'.repeat(64);
    });
    const report = diagnose(destination);
...
test('doctor CLI emits parseable JSON, stable exit codes and rejects irrelevant options', (t) => {
  ...
  for (const invalid of [
    ['doctor', '--tool', 'codex'],
    ['doctor', '--dry-run'],
    ['doctor', '--modules', 'scoped-delivery'],
    ['init', '--tool', 'codex', '--json'],
  ]) {
