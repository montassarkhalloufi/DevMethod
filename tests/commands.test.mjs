...
for (const tool of Object.keys(tools))
  test(`native commands: ${tool} installs all documented entry points and their references`, (t) => {
      ...
      ...fs
        .readFileSync(
          path.join(
            destination,
            tools[tool],
            'project-foundation/references/operating-commands.md',
          ),
          'utf8',
        )
        .matchAll(/^\| `devmethod-([a-z-]+)/gm),
    ].map((m) => m[1]);
    ...
    for (const stage of stages) {
      ...
      for (const [, target] of content.matchAll(/\]\(([^)]+)\)/g)) {
        assert.ok(
          fs.statSync(path.resolve(destination, path.dirname(file), target)).isFile(),
          target,
        );
      }
    }
...
test('subsets expose only supported procedures and reject arbitrary manifest command paths', (t) => {
  ...
  assert.deepEqual(fs.readdirSync(path.join(destination, '.agents/skills')).sort(), [
