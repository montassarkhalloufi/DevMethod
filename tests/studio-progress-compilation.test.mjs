...
test('React compilation failure is reported before an explicit terminal job failure', async (t) => {
  const f = fixture(t, true);
  await assert.rejects(f.finish(), /not assignable/);
  ...
  f.jobs.fail({ jobId: f.job.id, error: 'TypeScript a refusé la source.' });
  assert.equal(f.jobs.progress(f.job.id).status, 'failed');
...
test('an unavailable or exhausted progress journal cannot mask the compiler error or prevent delivery', async (t) => {
  for (const invalid of [true, false]) {
    ...
    if (invalid) await assert.rejects(f.finish(), /not assignable/);
