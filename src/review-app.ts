...
interface ViewerState {
  view: 'findings' | 'coverage' | 'sources';
  selected: string;
  section: 'evidence' | 'correction' | 'references';
...
function exports(parent: HTMLElement): void {
  ...
  menu.append(
    button('HTML interactif · rouvrir hors ligne', '', () => {
      box.open = false;
      exportHtml();
    }),
  );
  if (review) {
    ...
    menu.append(
      button('Rapport Markdown original', '', () =>
        download('review-legacy.md', legacy!, 'text/markdown;charset=utf-8'),
      ),
    );
  box.append(menu);
  parent.append(box);
...
function importer(parent: HTMLElement): void {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      ...
      legacy = null;
      showError(
        'Résultats invalides ou version non supportée. Utilisez un document review format 1 ; aucune donnée du fichier n’a été exécutée.',
      );
    } finally {
...
function shell(): HTMLElement {
  ...
  aside.append(
    el('div', 'brand', '⬡  DevMethod'),
    el('p', 'eyebrow', 'PROJET'),
    el('strong', 'project', review?.project || 'Consultation locale'),
  );
  ...
  note.append(
    el('strong', '', 'Vos données restent locales'),
    el('p', '', 'Aucune commande exécutée. Aucune ressource distante chargée à l’ouverture.'),
  );
  aside.append(note);
  ...
  const title = el('div');
  title.append(
    el('h1', '', review?.title || (legacy !== null ? 'Rapport historique' : 'Espace de review')),
  );
  if (review) {
    title.append(
      el(
        'p',
        'subtitle',
        `${review.project}  /  ${review.mission}  ·  ${review.tickets.map((t) => t.id).join(', ') || 'Sans ticket'}`,
      ),
      el(
        'p',
        'revision',
        `Révision ${review.revision.commit} · ${review.date}${review.revision.dirty.length ? ` · Modifications non commitées : ${review.revision.dirty.join(', ')}` : ''}`,
      ),
    );
  }
  header.append(title);
  if (review || legacy !== null) exports(header);
  main.append(header);
...
function render(): void {
    ...
    if (legacy !== null) {
      main.append(
        el(
          'div',
          'banner incomplete',
          'ⓘ Rapport Markdown historique — champs structurés et couverture inconnus. Aucune conversion implicite.',
        ),
        el('pre', 'legacy', legacy),
      );
    } else {
      const box = el('section', 'empty');
      box.append(
        el('span', 'empty-icon', '▤'),
        el('h2', '', 'Aucune review disponible'),
        el(
          'p',
          '',
          'Ouvrez un fichier review.json validé ou un ancien rapport Markdown pour commencer.',
        ),
      );
      importer(box);
      main.append(box);
  ...
  left.append(
    tabs(
      ...
      state.view,
      (id) => {
        state.view = id as ViewerState['view'];
  ...
  if (state.view === 'coverage') renderCoverage(r, panel);
  if (state.view === 'sources') renderSources(r, panel);
  left.append(panel);
...
function renderFindingList(parent: HTMLElement): void {
  ...
  const select = (key: keyof ReviewFilters, label: string, values: [string, string][]) => {
  ...
  select(
    'domain',
    'Domaine',
    [...new Set(review!.findings.map((f) => f.domain))].sort().map((v) => [v, v]),
  );
...
function renderListOnly(): void {
  ...
  if (list) fillFindingResults(list);
...
function fillFindingResults(parent: HTMLElement): void {
  ...
  const count = el(
    'p',
    'result-count',
    `${findings.length} constat(s) affiché(s) sur ${review!.findings.length}`,
  );
  ...
  for (const f of findings) {
    ...
    b.setAttribute(
      'aria-label',
      `${f.id} ${f.title}, ${severityLabels[f.severity]}, ${confidenceLabels[f.confidence]}, ${resolutionLabels[f.resolution]}`,
    );
    b.setAttribute('aria-current', String(state.selected === f.id));
    const top = el('div', 'finding-top');
    top.append(
      el('strong', 'finding-id', f.id),
      badge(confidenceLabels[f.confidence], f.confidence, f.confidence === 'confirmed' ? '✓' : '?'),
    );
    b.append(top, el('h3', '', f.title));
    const meta = el('div', 'finding-meta');
    meta.append(
      badge(severityLabels[f.severity], f.severity, '!'),
      el('span', '', f.domain),
      el('span', 'resolution', resolutionLabels[f.resolution]),
    );
    b.append(meta);
    parent.append(b);
  }
...
try {
  ...
  if (
    ...
    saved.filters &&
    ['query', 'domain', 'severity', 'confidence', 'resolution'].every(
      (k) => typeof saved.filters[k as keyof ReviewFilters] === 'string',
    )
  )
    state = saved;
  ...
  legacy = null;
  showError(
    'Résultats invalides ou version non supportée. Ouvrez une review format 1 ou un rapport Markdown historique.',
  );
}
