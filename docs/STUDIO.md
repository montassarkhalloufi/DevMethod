...
### Configuration et reprise locale de l’agent

Le panneau Agent permet de sonder l’installation Codex et son type d’accès, puis d’activer
les appels avec des limites enregistrées. Une connexion ChatGPT existante ne demande pas
une clé API. Aucun changement automatique de facturation. Les tokens connus constituent
un seuil entre appels ; un timeout sans bilan suspend les appels avec usage inconnu.

Dans les réglages du projet, la technologie attendue peut être enregistrée explicitement.
Le profil React/TypeScript strict doit alors correspondre aux sources livrées. Les contrôles
techniques précèdent l’adoption ; syntaxe/compilation réussies ne prouvent pas les critères métier.
Les candidats React échoués conservent leurs sources et diagnostics pour correction.

Le runner natif conserve une décision de contrôle et peut effectuer une seule correction
technique déléguée, depuis le candidat défaillant, dans les limites déjà enregistrées.
Vérifications affiche preuves liées, risque/inconnues, interventions et décision. Un candidat
arrêté peut être écarté explicitement pour libérer les demandes en attente ; cela ne lève pas
une suspension fournisseur et ne supprime pas les preuves.

Dans Historique, **Corriger cette version / Request changes** prépare une demande
depuis les fichiers du candidat affiché, sans l’activer. Le dialogue identifie la
version et conserve une saisie distincte du brouillon principal. Actualisez après
un conflit puis confirmez à nouveau. Une demande liée au candidat bloquant peut
passer avant les demandes ordinaires conservées, sans modifier leurs bases.
Le budget, les permissions et les arrêts restent applicables ; la filiation conserve
aussi les corrections automatiques échouées sans nouvelle version. Voir
[le contrat](ADR-038-explicit-candidate-request.md).

Dans Vérifications, **Points à examiner / Items to review** regroupe les observations
de la version évaluée par type et nombre. Un résultat sans succès courant attesté
par Studio reste à examiner ; les preuves d’autres versions sont comptées séparément.
Les nombres décrivent les observations disponibles, sans estimer le risque ni compter
une preuve manquante comme une observation. Les arrêts et facteurs critiques restent
visibles avant les groupes, même lorsqu’ils ne concernent qu’un seul point. Les liens
ouvrent les preuves existantes ; cet affichage ne donne aucun accord et ne lance rien.

Dans l’activité d’une demande arrêtée, « Vérifier le travail conservé sans relancer Codex »
crée une reprise locale contrôlée. La demande d’origine, ses fichiers et le registre restent
intacts. Le résultat demeure une candidate à examiner ; aucun appel ni accord humain ne sont
inventés. La reprise est refusée si le contexte ou la version de départ ont changé.
