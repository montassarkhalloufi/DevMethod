...
export const limitations = 'Experimental local evidence, not semantic acceptance or a security sandbox. Evaluator code and session storage are trusted. Only declared bytes are bound; undeclared dependencies, external writers and TOCTOU are outside this boundary. Controls measure declared fault sensitivity, not test independence or requirement completeness.';
function requireValue(condition, message) {
    if (!condition)
        throw new Error(message);
}
function keys(record, allowed) {
    requireValue(Object.keys(record).every((key) => allowed.includes(key)), 'Unknown contract field.');
}
function paths(value) {
    requireValue(Array.isArray(value) && value.length > 0 && value.length <= 512, 'Expected declared files.');
    requireValue(value.every((file) => safePath(file) && !secretPath(file)), 'Unsafe declared path.');
    requireValue(new Set(value).size === value.length, 'Duplicate declared file.');
    return value;
}
function parseCheck(value, criteria, files) {
    requireValue(object(value), 'Expected check.');
    keys(value, ['id', 'runner', 'criteria', 'healthy', 'faults']);
    requireValue(id(value.id) && typeof value.runner === 'string' && files.includes(value.runner), 'Invalid check runner or id.');
    requireValue(Array.isArray(value.criteria) && value.criteria.length > 0, 'Check needs criteria.');
    requireValue(value.criteria.every((item) => typeof item === 'string' && criteria.has(item)), 'Unknown check criterion.');
    requireValue(new Set(value.criteria).size === value.criteria.length, 'Duplicate check criterion.');
    requireValue(value.healthy === undefined || (id(value.healthy) && value.healthy !== 'candidate'), 'Invalid healthy mode.');
    ...
    for (const fault of faults) {
        ...
        requireValue(typeof fault.target === 'string' && value.criteria.includes(fault.target), 'Invalid fault target.');
        modes.add(fault.id);
    }
    return {
        id: value.id,
...
function limits(value) {
    ...
    for (const field of Object.keys(defaults)) {
        ...
        requireValue(Number.isSafeInteger(number) && Number(number) >= 1 && Number(number) <= maximum[field], `Invalid limit: ${field}.`);
...
export function validateEvidenceContract(value) {
    ...
    requireValue(value.format === 1 && id(value.id) && text(value.intent), 'Invalid contract identity.');
    requireValue(Array.isArray(value.criteria) && value.criteria.length > 0 && value.criteria.length <= 64, 'Expected 1–64 criteria.');
    ...
    requireValue(candidateInputs.length + evaluatorFiles.length <= 512, 'Declared file count exceeds 512.');
    ...
    requireValue(new Set(checks.map((check) => check.id)).size === checks.length, 'Duplicate check id.');
    requireValue(checks.reduce((sum, check) => sum + 1 + Number(!!check.healthy) + check.faults.length, 0) <= 64, 'More than 64 child invocations.');
    return {
        format: 1,
        id: value.id,
        intent: value.intent,
...
export function canonicalOptions(options) {
    ...
    for (const target of [root, evaluatorRoot, contractPath])
        checkPath(target);
    requireValue(fs.statSync(root).isDirectory() && fs.statSync(evaluatorRoot).isDirectory(), 'Expected candidate and evaluator directories.');
    requireValue(!inside(root, evaluatorRoot) && !inside(evaluatorRoot, root), 'Evaluator must be separate from candidate.');
    requireValue(!inside(root, contractPath), 'Contract must be outside candidate.');
    return { root, evaluatorRoot, contractPath };
}
export function boundedFile(file, maximum) {
    checkPath(file);
...
export function inspectEvidence(raw) {
    ...
    const engine = digest(JSON.stringify([
        process.version,
        hashFileSha256(process.execPath),
        ...engineFiles.map((file) => hashFileSha256(fileURLToPath(new URL(file, import.meta.url)))),
    ]));
    const permit = digest(JSON.stringify([options, digest(bytes), filesSignature(evaluator), engine]));
    const controls = contract.checks.flatMap((check) => [...(check.healthy ? [check.healthy] : []), ...check.faults.map((fault) => fault.id)].map((mode) => ({ check: check.id, runner: check.runner, mode })));
    const invocations = [
        ...controls,
