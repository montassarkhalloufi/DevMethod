import { CATEGORY_LABELS, applyCancellation, applyRegistration, initialData, normalizeData, placesLeft } from "./lib.mjs";
...
async function requestData() {
  ...
  if (payload.data != null && !data) throw new Error("Les données existantes ont un format incompatible ; elles n’ont pas été remplacées.");
...
async function register(workshopId) {
  ...
  try {
    const nextData = applyRegistration(state.data, workshopId, name);
    ...
    persistDrafts();
    announce("Inscription enregistrée.");
    render();
  } catch (error) {
...
async function cancel(registrationId) {
  ...
  try {
    const nextData = applyCancellation(state.data, registrationId);
    const result = await save(nextData);
    if (result === "conflict") {
      const fresh = await requestData();
      state.version = fresh.version;
      ...
      render();
      if (state.data.registrations.some(item => item.id === registrationId)) showCancelRetry(registrationId);
      else announce("Cette inscription avait déjà été annulée dans la version rechargée.");
      return;
    } else announce("Inscription annulée.");
    render();
  } catch (error) {
...
function renderWorkshop(workshop) {
  ...
  openButton.disabled = remaining === 0;
  if (remaining === 0) openButton.textContent = "Complet";
...
function renderRegistrations() {
  ...
  registrations.forEach(registration => {
    ...
    meta.textContent = `${workshop.time}, ${workshop.location}`;
    details.append(title, meta);
    const button = document.createElement("button");
    button.className = "cancel-button";
    button.type = "button";
    button.textContent = "Annuler";
    button.addEventListener("click", () => cancel(registration.id));
...
function showCancelRetry(registrationId) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "retry-button status-retry";
  button.textContent = "Réessayer l’annulation";
  button.addEventListener("click", () => cancel(registrationId));
...
function setBusy(workshopId, busy) {
  ...
  button.textContent = busy ? "Enregistrement…" : "Confirmer";
}
