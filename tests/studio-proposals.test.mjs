...
test('comparison exposes only checks for the exact implemented revision and never marks an image implemented', () => {
  ...
  assert.equal(comparison.options[0].checks.length, 2);
  assert.equal(comparison.options[0].checks[0].revisionId, 'second');
  assert.ok(comparison.options[0].checks.every((check) => check.revisionId === 'second'));
  assert.equal(comparison.options[0].checks.at(-1).status, 'failed');
  assert.deepEqual(comparison.options[1].checks, []);
  assert.equal(comparison.options[1].preview.status, 'simulation');
  comparison.options[0].checks.at(-1).status = 'passed';
...
test('a refused JavaScript candidate keeps accompanying proposals on the active base', () => {
  const state = fixture();
  state.project.delegation.adoption = 'agent';
  const job = queueRequest(state, { request: 'Candidate with decision' });
  claimJob(state, { worker: 'fixture' });
  finishJob(
    state,
    {
      ...
      revision: {
        ...
        jobId: job.id,
        title: 'Invalid',
        summary: 'Controlled syntax failure',
        createdAt: new Date().toISOString(),
        files: [
          { path: 'index.html', sha256: 'a'.repeat(64), bytes: 12 },
          { path: 'app.js', sha256: 'b'.repeat(64), bytes: 12 },
        ],
      },
      proposals: [input()],
    },
    {
      checks: [
        {
          label: 'Controlled syntax failure',
          kind: 'command',
          command: 'node --check',
  ...
  assert.equal(state.activeRevision, 'first');
  assert.equal(state.proposals[0].baseRevision, 'first');
  selectProposalOption(state, { proposalId: 'proposal', optionId: 'compact' });
  assert.equal(state.proposals[0].selectedOptionId, 'compact');
  validateStudioState(state);
});
