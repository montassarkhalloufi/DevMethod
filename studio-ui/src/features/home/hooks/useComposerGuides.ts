...
export function useComposerGuides({
  ...
  const restored: Record<string, GuideDraft> = Object.fromEntries(
  ...
  const displayedPreparation =
    validation.preparation && same(validation.preparation.input, current?.input)
  ...
  async function prepare(input: GuideInput) {
    ...
    setDrafts((previous) => {
      ...
      return {
        ...previous,
        [input.optionId]: { input: result.input, preparation: result, locale },
      };
    });
  }
  function apply(preparation: GuidePreparation) {
    if (busy || displayedPreparation !== preparation) return;
