...
test('source-only editing creates a candidate snapshot, preserves baseline until approval and records no fake check', async (t) => {
  ...
  assert.equal(
    readActivationReview(f.store, { revisionId: result.revision.id }).canActivate,
    false,
  );
  ...
  const review = readActivationReview(f.store, { revisionId: result.revision.id });
