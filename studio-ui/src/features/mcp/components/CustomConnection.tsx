import { useI18n } from '../../../i18n';
import { connectorText, connectorMessage } from '../../connectors/model/i18n';
import { useRef, useState } from 'react';
import type { McpConnectionsController } from '../hooks/useMcpConnections';
import { mcpConnectionInput } from '../model/mcp';
import type { McpAuth, McpConnection } from '../model/mcp';

export function CustomConnection({
  controller,
  disabled,
  connection,
  onDismiss,
}: {
  controller: McpConnectionsController;
  disabled: boolean;
  connection?: McpConnection;
  onDismiss?(): void;
}) {
  const { locale } = useI18n();
  const [open, setOpen] = useState(Boolean(connection));
  const [auth, setAuth] = useState<McpAuth>(connection?.auth || 'oauth');
  const [error, setError] = useState('');
  const form = useRef<HTMLFormElement>(null);
  return (
    <div className="mcp-custom-connection">
      {!connection ? (
        <button
          type="button"
          className="mcp-add-custom"
          disabled={disabled}
          aria-expanded={open}
          onClick={() => setOpen(!open)}
        >
          {connectorText('+ Ajouter un serveur personnalisé', locale)}
        </button>
      ) : null}
      {open ? (
        <form
          ref={form}
          onSubmit={(event) => {
            event.preventDefault();
            if (disabled || controller.active) return;
            try {
              const input = mcpConnectionInput(event.currentTarget);
              setError('');
              void controller.connect({ ...input, ...(connection ? { id: connection.id } : {}) });
              const token = event.currentTarget.elements.namedItem('mcp-token');
              if (token instanceof HTMLInputElement) token.value = '';
            } catch (cause) {
              setError(
                cause instanceof Error ? cause.message : 'Vérifiez les informations du serveur.',
              );
            }
          }}
        >
          <input type="hidden" name="mcp-provider" value="custom" />
          <div className="mcp-custom-fields">
            <label>
              {connectorText('Nom du serveur', locale)}
              <input
                name="mcp-name"
                defaultValue={connection?.name}
                maxLength={100}
                autoComplete="off"
                required
                disabled={disabled || Boolean(controller.active)}
                placeholder={connectorText('Mon espace documentaire', locale)}
              />
            </label>
            <label>
              {connectorText('Adresse MCP', locale)}
              <input
                name="mcp-url"
                defaultValue={connection?.url}
                type="url"
                maxLength={2048}
                autoComplete="off"
                required
                disabled={disabled || Boolean(controller.active)}
                placeholder={connectorMessage(
                  'https://serveur.exemple/mcp',
                  'https://server.example/mcp',
                  locale,
                )}
              />
            </label>
          </div>
          <label>
            {connectorText('Authentification', locale)}
            <select
              name="mcp-auth"
              value={auth}
              disabled={disabled || Boolean(controller.active)}
              onChange={(event) => setAuth(event.target.value as McpAuth)}
            >
              <option value="oauth">
                {connectorMessage(
                  'OAuth · autoriser dans le navigateur',
                  'OAuth · authorize in browser',
                  locale,
                )}
              </option>
              <option value="bearer">{connectorText('Jeton Bearer', locale)}</option>
              <option value="none">{connectorText('Sans authentification', locale)}</option>
            </select>
          </label>
          {auth === 'bearer' ? (
            <label>
              {connectorText('Jeton de connexion', locale)}
              <input
                name="mcp-token"
                type="password"
                maxLength={8192}
                autoComplete="off"
                spellCheck={false}
                required
                disabled={disabled || Boolean(controller.active)}
              />
              <small>
                {connectorText(
                  'Transmis au gestionnaire local. Il n’est jamais ajouté à votre demande.',
                  locale,
                )}
              </small>
            </label>
          ) : null}
          {error ? (
            <p role="alert" className="mcp-connection-error">
              {connectorText(error, locale)}
            </p>
          ) : null}
          <div className="mcp-form-actions">
            <button
              type="button"
              disabled={Boolean(controller.active)}
              onClick={() => {
                setOpen(false);
                onDismiss?.();
              }}
            >
              {connectorText('Fermer les réglages', locale)}
            </button>
            <button
              type="submit"
              className="primary"
              disabled={disabled || Boolean(controller.active)}
            >
              {connectorText('Connecter le serveur', locale)}
            </button>
          </div>
        </form>
      ) : null}
    </div>
  );
}
