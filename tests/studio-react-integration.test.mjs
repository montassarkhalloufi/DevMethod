...
test('React job, edited type error, recovery, adoption, export and real server restart preserve source and data', async (t) => {
  ...
  assert.equal(adopted.activated, false);
  const review = readActivationReview(f.studio.store, { revisionId: adopted.revision.id });
  activateReviewedRevision(f.studio.store, {
    id: adopted.revision.id,
    version: review.version,
    reviewKey: review.reviewKey,
    reason: 'Typed candidate reviewed',
  });
