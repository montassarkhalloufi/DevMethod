...
test('corrupt quality history stops autonomy while Studio remains available for inspection', async (t) => {
  const { root, store, jobs, claim } = activeWork(t);
  await jobs.finish({ jobId: claim.job.id, title: 'Candidate' });
  store.close();
  const quality = path.join(root, '.devmethod', 'quality');
  fs.mkdirSync(quality, { recursive: true });
  fs.writeFileSync(path.join(quality, '11111111-1111-1111-1111-111111111111.json'), '{');
  const studio = await startStudio({ workspace: root, port: 0 });
  t.after(() => studio.close());
  const response = await fetch(studio.runtime().url + '/api/runtime');
  assert.equal(response.status, 200);
  const runtime = await response.json();
  assert.equal(runtime.control.autonomy.action, 'stop');
  assert.ok(runtime.control.autonomy.reasons.includes('context-changed'));
  assert.equal((await fetch(studio.runtime().url + '/')).status, 200);
});
