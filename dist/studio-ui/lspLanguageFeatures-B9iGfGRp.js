import { c as e, i as t, l as n, o as r, s as i } from "./editor.api-CBhnZFl3.js";
...
var He = class {
	constructor(t, n, r) {
		this._languageId = t, this._worker = n, this._disposables = [], this._listener = /* @__PURE__ */ Object.create(null);
		let i = (e) => {
			...
			e.setModelMarkers(t, this._languageId, []);
			let n = t.uri.toString(), r = this._listener[n];
			r && (r.dispose(), delete this._listener[n]);
		};
		this._disposables.push(e.onDidCreateModel(i)), this._disposables.push(e.onWillDisposeModel(a)), this._disposables.push(e.onDidChangeModelLanguage((e) => {
			a(e.model), i(e.model);
		})), this._disposables.push(r((t) => {
			e.getModels().forEach((e) => {
				e.getLanguageId() === this._languageId && (a(e), i(e));
			});
		})), this._disposables.push({ dispose: () => {
			e.getModels().forEach(a);
			for (let e in this._listener) this._listener[e].dispose();
		} }), e.getModels().forEach(i);
	}
	dispose() {
		this._disposables.forEach((e) => e && e.dispose()), this._disposables.length = 0;
	}
	_doValidate(t, n) {
		this._worker(t).then((e) => e.doValidation(t.toString())).then((r) => {
			let i = r.map((e) => We(t, e)), a = e.getModel(t);
			a && a.getLanguageId() === n && e.setModelMarkers(a, n, i);
		}).then(void 0, (e) => {
			console.error(e);
		});
	}
...
function Ue(e) {
	switch (e) {
		case g.Error: return t.Error;
		case g.Warning: return t.Warning;
		case g.Information: return t.Info;
		case g.Hint: return t.Hint;
...
var Ge = class {
	...
	provideCompletionItems(e, t, i, a) {
		let o = e.uri;
		return this._worker(o).then((e) => e.doComplete(o.toString(), W(t))).then((i) => {
			if (!i) return;
			let a = e.getWordUntilPosition(t), o = new r(t.lineNumber, a.startColumn, t.lineNumber, a.endColumn), s = i.items.map((e) => {
				let t = {
					label: e.label,
				...
				return e.textEdit && (t.range = Ke(e.textEdit) ? {
					insert: K(e.textEdit.insert),
					replace: K(e.textEdit.replace)
				} : K(e.textEdit.range), t.insertText = e.textEdit.newText), e.additionalTextEdits && (t.additionalTextEdits = e.additionalTextEdits.map(q)), e.insertTextFormat === N.Snippet && (t.insertTextRules = n.CompletionItemInsertTextRule.InsertAsSnippet), t;
			});
			return {
...
function K(e) {
	if (e) return new r(e.start.line + 1, e.start.character + 1, e.end.line + 1, e.end.character + 1);
}
function Ke(e) {
	return e.insert !== void 0 && e.replace !== void 0;
}
function qe(e) {
	let t = n.CompletionItemKind;
	switch (e) {
		case M.Text: return t.Text;
		case M.Method: return t.Method;
		case M.Function: return t.Function;
		case M.Constructor: return t.Constructor;
		case M.Field: return t.Field;
		case M.Variable: return t.Variable;
		case M.Class: return t.Class;
		case M.Interface: return t.Interface;
		case M.Module: return t.Module;
		case M.Property: return t.Property;
		case M.Unit: return t.Unit;
		case M.Value: return t.Value;
		case M.Enum: return t.Enum;
		case M.Keyword: return t.Keyword;
		case M.Snippet: return t.Snippet;
		case M.Color: return t.Color;
		case M.File: return t.File;
		case M.Reference: return t.Reference;
	}
	return t.Property;
}
function q(e) {
...
function $e(e) {
	switch (e) {
		case I.Read: return n.DocumentHighlightKind.Read;
		case I.Write: return n.DocumentHighlightKind.Write;
		case I.Text: return n.DocumentHighlightKind.Text;
	}
...
function Q(e) {
	let t = n.SymbolKind;
	switch (e) {
		case R.File: return t.File;
		case R.Module: return t.Module;
		case R.Namespace: return t.Namespace;
		case R.Package: return t.Package;
		case R.Class: return t.Class;
		case R.Method: return t.Method;
		case R.Property: return t.Property;
		case R.Field: return t.Field;
		case R.Constructor: return t.Constructor;
		case R.Enum: return t.Enum;
		case R.Interface: return t.Interface;
		case R.Function: return t.Function;
		case R.Variable: return t.Variable;
		case R.Constant: return t.Constant;
		case R.String: return t.String;
		case R.Number: return t.Number;
		case R.Boolean: return t.Boolean;
		case R.Array: return t.Array;
	}
	return t.Function;
}
var at = class {
...
function ut(e) {
	switch (e) {
		case m.Comment: return n.FoldingRangeKind.Comment;
		case m.Imports: return n.FoldingRangeKind.Imports;
		case m.Region: return n.FoldingRangeKind.Region;
	}
}
var dt = class {
	constructor(e) {
