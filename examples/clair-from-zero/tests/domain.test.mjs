...
const a = createTask('Lire', 'Une note', 'a'),
  b = createTask('Marcher', '', 'b');
test('creation trims inputs; rejects blank and over-limit values', () => {
  assert.deepEqual(createTask('  Titre  ', ' Note ', 'id'), {
    id: 'id',
    title: 'Titre',
    note: 'Note',
    done: false,
  ...
  for (const title of ['', '   ', '\n\t', 'x'.repeat(161)])
    assert.throws(() => createTask(title, '', 'id'));
...
test('toggle is immutable and filters preserve order', () => {
  const initial = [a, b],
    changed = toggleTask(initial, 'a');
  assert.equal(a.done, false);
  assert.equal(changed[0].done, true);
  assert.deepEqual(filterTasks(changed, 'done'), [changed[0]]);
  assert.deepEqual(filterTasks(changed, 'active'), [b]);
  assert.deepEqual(filterTasks(changed, 'all'), changed);
  assert.deepEqual(toggleTask(changed, 'a'), initial);
});
test('delete and undo restore original position even after adding another task', () => {
  const initial = [a, b],
    { tasks, removed } = removeTask(initial, 'a'),
    c = createTask('Écrire', '', 'c');
  assert.deepEqual(tasks, [b]);
  assert.deepEqual(restoreTask([...tasks, c], removed), [a, b, c]);
  assert.deepEqual(restoreTask(initial, removed), initial);
  assert.equal(removeTask(initial, 'missing').removed, null);
...
test('untrusted storage schema rejects invalid data and duplicate IDs', () => {
  for (const raw of [
    ...
    JSON.stringify({ version: 1, tasks: [a, a] }),
    JSON.stringify({ version: 1, tasks: [{ ...a, done: 'true' }] }),
    JSON.stringify({ version: 1, tasks: [{ ...a, title: ' ' }] }),
    JSON.stringify({ version: 1, tasks: [{ ...a, note: null }] }),
  ])
...
test('storage adapter uses versioned key; failures propagate rather than fake success', () => {
  ...
  writeTasks(storage, [a, b]);
  assert.ok(map.has(STORAGE_KEY));
  assert.deepEqual(readTasks(storage), [a, b]);
  assert.throws(
    () =>
      writeTasks(
        {
