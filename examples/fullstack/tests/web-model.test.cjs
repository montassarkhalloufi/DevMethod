...
const code = ts.transpileModule(
  readFileSync(resolve(__dirname, '../web/features/tasks/model/tasks.ts'), 'utf8'),
  {
...
test('filtering is normalized and does not mutate server data', () => {
  const tasks = Object.freeze([
    Object.freeze({ id: '1', title: 'Review ADR' }),
    Object.freeze({ id: '2', title: 'Ship fixture' }),
  ]);
...
test('the server boundary rejects malformed data and strips unknown fields', () => {
  for (const value of [
    ...
    {},
    [{ id: 12, title: 'wrong' }],
    new Array(101).fill({ id: '1', title: 'x' }),
  ]) {
    assert.throws(() => parseTasks(value), /Unexpected tasks response/);
  }
  assert.deepEqual(parseTasks([{ id: '1', title: 'Read ADR', secret: 'never serialize' }]), [
    { id: '1', title: 'Read ADR' },
  ]);
});
