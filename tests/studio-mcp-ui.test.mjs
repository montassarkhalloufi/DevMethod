...
function fixture(t, handler, { studio = false, popupBlocked = false } = {}) {
  ...
  w.fetch = async (url, init) => {
    ...
    if (url === '/api/connectors/guides?language=fr') return reply(readConnectorGuides('fr'));
    if (url === '/api/connectors/guides/prepare?language=fr')
      return reply(prepareConnectorGuide(input));
    if (url === '/api/home') return reply({ projects: [], limits: { projects: 200 } });
    if (url === '/api/home/catalog?language=fr') return reply({ options: [], capabilities: [] });
    return handler(url, input, init);
  };
  w.document.documentElement.lang = 'fr';
  w.document.documentElement.setAttribute('data-studio-language-ready', '');
