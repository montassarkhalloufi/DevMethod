...
export function compareLineSources(
  before,
  after,
  { maxCells = 2_000_000, maxCharacters = 524_288, maxLines = 12_000, locale = getLocale() } = {},
) {
  const t = (fr, en) => translate(fr, en, {}, locale);
  if (typeof before !== 'string' || typeof after !== 'string')
    throw new TypeError(
      t(
        'Deux contenus textuels complets sont nécessaires.',
        'Two complete text files are required.',
      ),
    );
  if (before.length + after.length > maxCharacters)
    return {
      kind: 'limited',
      reason: t(
        'Les fichiers dépassent la limite de taille de comparaison.',
        'The files exceed the comparison size limit.',
      ),
    };
  ...
  if (oldLines.length + newLines.length > maxLines)
    return {
      kind: 'limited',
      reason: t(
        'Les fichiers contiennent trop de lignes pour cet aperçu.',
        'The files contain too many lines for this preview.',
      ),
    };
  ...
  if (oldCount && newCount && (oldCount + 1) * (newCount + 1) > maxCells)
    return {
      kind: 'limited',
      reason: t(
        'La comparaison dépasse le budget de calcul de cet aperçu.',
        'The comparison exceeds this preview’s computation budget.',
      ),
    };
