...
async function checkEditing(runtime, revision, source, originalData) {
  ...
  assert.equal(applied.state.activeRevision, revision.id);
  const review = await request(
    runtime.url,
    '/api/activation-review?revision=' + applied.revision.id,
  );
  const adoption = await request(runtime.url, '/api/activate', {
    version: review.version,
    id: applied.revision.id,
    reviewKey: review.reviewKey,
    reason: 'Packaged editor candidate explicitly reviewed',
  });
  applied.state = adoption.state;
  ...
  assert.ok(
    checks.some(
      (check) => check.executor === 'studio' && check.protocol === 'studio-document-syntax-v1',
    ),
  );
  assert.ok(
    checks.some(
      (check) => check.executor === 'studio' && check.protocol === 'studio-javascript-syntax-v1',
    ),
  );
