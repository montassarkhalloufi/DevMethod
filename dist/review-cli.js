...
function validateOutputs(root, outputs, options) {
...
export function prepareReview(options) {
    ...
    const read = (file) => {
        if (secretPath(file))
            throw new Error('Secret-like review paths are excluded.');
        return readLocal(root, file, 4 * 1024 * 1024).toString('utf8');
    };
    const source = options.demo
        ? fs.readFileSync(fileURLToPath(new URL('../examples/review/review.json', import.meta.url)), 'utf8')
        : options.review
            ? read(options.review)
            : null;
...
function reviewResult(root, options, review, legacy, outputs) {
    ...
    return {
        ...
        limitations: 'Explicit record only; no review execution or filesystem evidence discovery. Legacy Markdown has no inferred fields. Inspect privacy before sharing; automatic redaction is heuristic.',
    };
}
