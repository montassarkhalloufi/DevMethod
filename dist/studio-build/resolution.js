...
export const normalize = (file) => path.resolve(file).replaceAll('\\', '/');
export function packageName(id) {
    return id.startsWith('@') ? id.split('/').slice(0, 2).join('/') : (id.split('/')[0] ?? id);
}
export function packageRoot(name, importer) {
    ...
    while (!fs.existsSync(path.join(directory, 'package.json'))) {
        ...
        if (parent === directory)
            throw new Error(`Bibliothèque absente : ${name}`);
        directory = parent;
    }
...
export function createResolution(input) {
    ...
    const isTrusted = (file) => roots.some((root) => inside(root, file));
    function read(file) {
        ...
        if (!isTrusted(file))
            return undefined;
        try {
            if (!isTrusted(fs.realpathSync(file)))
                return undefined;
    ...
    function local(id, importer) {
        let candidate;
        if (id.startsWith('@/'))
            candidate = path.join(sourceRoot, 'src', id.slice(2));
        else if (id.startsWith('/'))
            candidate = path.join(sourceRoot, id.slice(1));
        else
            candidate = path.resolve(path.dirname(importer), id);
        if (!inside(sourceRoot, candidate))
            throw new Error(`Import hors du projet refusé : ${id}`);
