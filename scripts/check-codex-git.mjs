...
export async function checkCodexGit() {
  ...
  fs.writeFileSync(cwd + '/sentinel.txt', 'fictional\n');
  const rpc = startCodexRpc({ cwd, timeoutMs: 20000 });
  const results = [];
  try {
    ...
    for (const includeGit of [false, true]) {
      const result = await rpc.call('command/exec', {
        command: ['git', 'add', 'sentinel.txt'],
        cwd,
        timeoutMs: 5000,
        sandboxPolicy: {
          type: 'workspaceWrite',
          writableRoots: includeGit ? [cwd, cwd + '/.git'] : [cwd],
          networkAccess: false,
    ...
    if (results[0].exitCode === 0 || results[1].exitCode !== 0)
      throw new Error('Unexpected Git sandbox result');
...
if (process.argv[1]?.endsWith('/check-codex-git.mjs'))
  console.log(JSON.stringify(await checkCodexGit()));
