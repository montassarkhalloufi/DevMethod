...
function checkJobTransition(previous, next) {
  ...
  if (previous.control !== undefined)
    unchanged(previous.control, next.control, 'Décision de contrôle');
  if (!['queued', 'running'].includes(previous.status)) {
    const { control: previousControl, ...previousResult } = previous;
    const { control: nextControl, ...nextResult } = next;
    unchanged(previousResult, nextResult, 'Résultat de demande');
