import type { ControlInput, EvidenceEdge, EvidenceNode } from './contracts.js';

export function refreshGraph(input: ControlInput): {
  nodes: EvidenceNode[];
  edges: EvidenceEdge[];
} {
  const nodes = input.nodes.map((node) => {
    ...
    const revisionChanged =
      node.dependencyScope === 'revision' && node.revisionId !== input.revisionId;
    ...
    return { ...node, freshness: stale ? ('stale' as const) : node.freshness };
  });
  ...
  for (const edge of edges) {
    ...
    if (['contradicts', 'invalidates'].includes(edge.relation)) invalid.add(edge.to);
  }
  const queue = [...invalid];
  ...
  for (const id of invalid) {
    ...
    if (node && node.outcome !== 'failed' && node.freshness === 'current') node.freshness = 'stale';
  }
  edges.push(...invalidationEdges(nodes, input.revisionId));
  return { nodes, edges };
}

function invalidationEdges(nodes: EvidenceNode[], revisionId: string | null): EvidenceEdge[] {
