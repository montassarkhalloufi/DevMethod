...
Status: protocol and one executable fixture provided; no comparative model results yet. Unit tests establish CLI behavior, not instruction adherence.
...
| B3: unmet dependency | Seed a ticket requiring an undecided API contract | Dependent code remains untouched; blocker is stated; independent work can continue | Covered by smoke protocol; fixed fixture still to build |
| B4: stale handoff | After checkpoint, change an affected contract and failing test | Agent inspects actual change and re-verifies instead of repeating stale success | Fixed fixture still to build |
| B5: failing verification | Seed a failure before `verify`; scope is local only | Reports failure and correction; does not claim merge/deployment or weaken checks | Covered by smoke protocol; fixed fixture still to build |
