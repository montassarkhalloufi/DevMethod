...
function GraphNode({
  ...
  const provenance =
    [...new Set(element.provenance.map((item) => PROVENANCE[item.kind]))].join(' · ') ||
  ...
  return (
    <g transform={`translate(${point.x},${point.y})`}>
      <g
        ...
        aria-label={`${element.label} · ${ELEMENT_LABELS[element.type]} · ${provenance}`}
      ...
      >
        <title>
          ...
          {'\n'}
          {provenance}
          {'\n'}Runtime : {element.runtime === 'observed' ? 'observé' : 'non observé'}
        </title>
        <rect
          width={CARD.width}
          height={CARD.height}
        ...
        <text
          ...
          fontSize={10}
        >
          {element.runtime === 'observed' ? '◉ Exécution observée' : '○ Exécution non observée'}
        </text>
        {change ? (
          <text
            x={CARD.width - 9}
      ...
      {source ? (
        <g
          ...
          className="arch-svg-action"
          aria-label={`Ouvrir le code de ${element.label}`}
...
function GraphEdge({
  ...
  return (
    <g
      ...
      className="arch-svg-action"
      aria-label={`${RELATION_LABELS[relation.kind]} : ${relation.label}`}
    ...
    >
      <title>
        {relation.label}
        {'\n'}
        {relation.provenance.map((item) => `${PROVENANCE[item.kind]} · ${item.method}`).join('\n')}
      </title>
...
export function ArchitectureGraph(props: GraphProps) {
  ...
  return (
    <svg
      ...
      {...canvas.handlers}
    >
      <title>Architecture du projet sélectionné</title>
      <desc>
        Carte issue de l’analyse du projet. Les connexions de code ne prouvent pas le fonctionnement
        à l’exécution. Une liste navigable est disponible sous la carte.
      </desc>
      <defs>
