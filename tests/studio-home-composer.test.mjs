...
function fixture(
  ...
  window.fetch = (url, init) => {
    ...
    if (url === '/api/connectors/guides?language=fr') return Promise.resolve(reply({ guides }));
    calls.push({ url, init });
...
function guideFetcher(url, init) {
  return Promise.resolve(
    reply(
      url.endsWith('/prepare?language=fr') ? preparedGuide(JSON.parse(init.body)) : guidedCatalog,
    ),
  );
}
...
test('guide preparation errors retain answers and an edited selected guide must be confirmed again before launch', async (t) => {
  ...
  const f = fixture(t, {
    ...
    fetcher: async (url, init) => {
      if (url.endsWith('/prepare?language=fr') && fail)
...
test('an assistant guide is separate from application services and adding it makes no MCP or business request', async (t) => {
  const f = fixture(t, {
    ...
    fetcher: async (url, init) =>
      reply(
        url.endsWith('/prepare?language=fr') ? preparedGuide(JSON.parse(init.body)) : notionCatalog,
      ),
  });
  await until(() => idea(f));
  ...
  assert.deepEqual(
    ...
    ['/api/connectors/guides/prepare?language=fr'],
  );
});
...
test('a prepared MCP guide invokes the native endpoint only on Connect and shows its returned status without losing the brief', async (t) => {
  ...
  const f = fixture(t, {
    ...
    fetcher: async (url, init) => {
      ...
      return reply(
        url.endsWith('/prepare?language=fr')
          ? {
...
test('catalog failure is retryable and tool search, categories and selection survive panel changes', async (t) => {
  ...
  assert.equal(f.calls[0].url, '/api/home/catalog?language=fr');
