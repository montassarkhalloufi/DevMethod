...
export const fixtures = JSON.parse(
  fs.readFileSync(path.join(root, 'evaluation/fixtures.json')),
).cases;
...
export function prepare(caseId, destination) {
  ...
  if (
    JSON.stringify(Object.entries(snapshot(source)).sort()) !==
    JSON.stringify(Object.entries(fixture.files).sort())
  )
    throw new Error('Fixture differs from pinned manifest');
...
export function collect(caseId, destination, baseline, verifier) {
  ...
  const changed = [...new Set([...Object.keys(baseline), ...Object.keys(current)])].filter(
  ...
  const check = testsIntact
    ? verifier
      ? verifier(destination, fixture.check.slice(1))
      : spawnSync(process.execPath, fixture.check.slice(1), {
          cwd: destination,
          env: environment,
          encoding: 'utf8',
          timeout: 30000,
  ...
  return {
    ...
    check: check
      ...
      : { skipped: 'Acceptance tests changed' },
    behavioralReview: 'pending',
    nativeHostEvidence: 'not established by this collector',
  };
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (
      !caseId ||
      !destination ||
      !['prepare', 'collect'].includes(command) ||
      (command === 'collect' && !baselineFile)
    )
      throw new Error(
        'Usage: node scripts/evaluation.mjs prepare CASE FRESH_DIR | collect CASE DIR BASELINE_JSON',
      );
    const result =
      command === 'prepare'
        ? prepare(caseId, destination)
        : collect(caseId, destination, JSON.parse(fs.readFileSync(baselineFile)).baseline);
