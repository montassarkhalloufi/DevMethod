...
The next CI run passed the full Linux/macOS pipelines. Windows then exposed a test-harness import defect: Node's `--import` needs a file URL, while the harness supplied a drive-letter path. [The native failure](ci-windows-import-failure.log) retains all eight failing case names. [A local red reproduction](import-url-red.log) adds `#` to the fixture path, exposing the same URL-conversion requirement on POSIX. The corrected harness uses `pathToFileURL(...).href`; all eight trace cases remain enabled on Windows. Runtime, application and demonstration code are unchanged by this final test correction.
