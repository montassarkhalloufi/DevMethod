...
import { ALLOWED_PACKAGES, type InputFile } from './types.js';
import { packageName, type Resolution } from './resolution.js';
import { compiledNotices } from './notices.js';

function loader(file: string): esbuild.Loader {
  const extension = path.extname(file).slice(1);
  if (['ts', 'tsx', 'js', 'jsx', 'json', 'css'].includes(extension))
    return extension as esbuild.Loader;
...
function resolver(context: Resolution): esbuild.Plugin {
  return {
    ...
    setup(build) {
      ...
      build.onLoad({ filter: /.*/, namespace: 'source' }, async (args) => {
        ...
        if (!bytes) throw new Error('Fichier absent de l’instantané.');
        const kind = loader(args.path);
        const contents =
          kind === 'css' ? await compileStyles(bytes.toString(), args.path, context) : bytes;
...
function compiledHTML(context: Resolution, css: boolean): string {
  const input = context.read(path.join(context.sourceRoot, 'index.html'));
  if (!input) throw new Error('index.html est requis.');
  ...
  if (
    ...
    throw new Error(
      'index.html doit contenir un seul script module src="/src/main.tsx" ; les scripts supplémentaires ne sont pas vérifiés.',
    );
  }
...
export async function bundle(context: Resolution): Promise<InputFile[]> {
  ...
  const html = compiledHTML(
    context,
    outputs.some((file) => file.path === 'assets/app.css'),
  );
  outputs.push({
    ...
    content: Buffer.from(
      compiledNotices(
        context,
        built.metafile,
        outputs.some((file) => file.path.endsWith('.css')),
      ),
    ).toString('base64'),
  });
