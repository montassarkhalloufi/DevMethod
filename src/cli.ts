...
try {
  ...
  if (values.help) console.log(help);
    ...
    if (positionals.length !== 1 || !['init', 'doctor'].includes(positionals[0] ?? '')) throw new Error(help);
    if (positionals[0] === 'doctor') {
      ...
      if (values.json !== undefined) throw new Error('--json is supported only by doctor');
