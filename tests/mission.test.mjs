...
function fixture(t) {
  ...
  const mission = {
    ...
    owner: 'fixture',
    status: 'active',
    nextAction: 'Review title validation',
    stopConditions: ['Acceptance passes'],
    acceptance: [
      {
        id: 'AC1',
        description: 'Reject empty titles',
        changes: ['contract.md'],
    ...
    sources: [
      {
        ...
        path: 'contract.md',
        level: 'domain',
        reason: 'Defines validation',
        authority: 'Accepted API contract for task titles',
...
test('mission context retains provenance without source bodies and detects changed contracts', (t) => {
  ...
  assert.equal(report.sources[0].state, 'changed');
...
test('Git changes include dirty-to-dirty edits and branch changes; complete never gains a next action', (t) => {
  ...
  fs.writeFileSync(path.join(root, 'check.log'), 'dirty A');
  const state = gitState(root);
  fs.writeFileSync(path.join(root, 'check.log'), 'dirty B');
  assert.notEqual(gitState(root).diffSha256, state.diffSha256);
...
test('dependencies and declared contradictions block without modifying records', (t) => {
  ...
  mission.dependencies.push({
    ...
    status: 'blocked',
    detail: 'Waiting for API decision',
  });
  ...
  mission.sources.push({ ...mission.sources[0], id: 'code', path: 'check.log', kind: 'fact' });
  mission.contradictions.push({
    sourceIds: ['contract', 'code'],
    detail: 'Code disagrees with contract',
    resolved: false,
  });
  assert.equal(missionStatus(validateMission(mission)), 'blocked');
});
test('secret paths, credential text, symlinks, oversized sources and tampered pins fail closed', (t) => {
  ...
  for (const file of [
    ...
    const value = structuredClone(mission);
    value.sources[0].path = file;
    assert.throws(() => captureContext(root, value));
  }
  const context = captureContext(root, mission);
  context.sources[0].reason = 'tampered';
  assert.throws(() => inspectContext(root, context));
  fs.writeFileSync(path.join(root, 'contract.md'), 'token = "12345678901234567890123456789"');
  assert.throws(() => captureContext(root, mission));
  fs.writeFileSync(path.join(root, 'contract.md'), 'x'.repeat(256 * 1024 + 1));
  assert.throws(() => captureContext(root, mission));
  fs.unlinkSync(path.join(root, 'contract.md'));
  fs.symlinkSync('check.log', path.join(root, 'contract.md'));
  assert.throws(() => captureContext(root, mission));
});
...
test('checkpoint extensions preserve legacy pins, blocked outcome, and selective Git reassessment', (t) => {
    ...
    sources: [
      {
        id: 'contract',
        path: 'contract.md',
        sha256: hash(fs.readFileSync(path.join(root, 'contract.md'))),
      },
    ],
    evidence: [
      {
  ...
  assert.equal(inspectCheckpoint(root, checkpoint).status, 'ready');
  git('checkout', '-b', 'changed');
  const report = inspectCheckpoint(root, checkpoint);
  assert.equal(report.status, 'reverify');
  assert.equal(report.evidence[0].state, 'valid');
  delete checkpoint.git;
  checkpoint.evidence[0].outcome = 'blocked';
  assert.equal(inspectCheckpoint(root, checkpoint).status, 'blocked');
  checkpoint.evidence[0].outcome = 'passed';
  checkpoint.blockers = ['API dependency unresolved'];
  assert.equal(inspectCheckpoint(root, checkpoint).status, 'blocked');
});
...
test('unknown mission/source properties are omitted from generated context', (t) => {
  const { root, mission } = fixture(t);
  mission.credentials = 'DO-NOT-COPY';
  mission.sources[0].content = 'DO-NOT-COPY';
  const context = captureContext(root, mission);
