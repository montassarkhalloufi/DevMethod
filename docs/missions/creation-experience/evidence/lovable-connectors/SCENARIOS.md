...
Brevo illustre une validation contextualisée : champ secret, aide pour obtenir la clé, erreur à proximité, aucun succès fictif. La capture de validation vide est conservée en privé car elle contient un libellé personnel ; voir la [note de provenance publique](../../../local-oss-v1/evidence/PUBLIC-EVIDENCE.md#lovable-account-label). Firecrawl montre une autre famille : service géré et conditions associées. Le nom d’un fournisseur ne permet pas de déduire son mécanisme de connexion.
...
- [Observations DOM/texte](guided-scenarios/observations.json) : dérivé public des captures de texte des formulaires et réponses IA, sans URL OAuth de session ; quatre occurrences du libellé personnel Brevo sont explicitement remplacées. Les clés de l’objet correspondent aux étapes et les autres octets sont conservés.
- Les quatre captures PNG originales ont été relues visuellement. Trois restent distribuées, celle de Brevo est conservée en privé. Elles attestent le rendu desktop observé, pas l’accessibilité complète ni le mobile ; le retrait ne constitue pas une nouvelle vérification.
