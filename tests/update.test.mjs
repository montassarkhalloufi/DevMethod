...
for (const tool of Object.keys(tools)) test(`preview ${tool} full/subset provenance is deterministic and read-only`, t => {
  for (const selected of [undefined, ['scoped-delivery']]) {
    ...
    assert.ok(report.entries.every(e => e.classification === 'unchanged' && !e.candidateChanged));
...
test('legacy manifest remains untouched and provenance is unknown', t => {
  ...
  edit(destination, m => { delete m.provenance; });
  const before = snapshot(destination);
...
test('comparison distinguishes upstream edits, local customization, additions, removals and collisions without writes', t => {
  ...
  assert.equal(previewUpdate(destination).entries.find(e => e.path === added).collision, undefined);
...
test('preview refuses traversal and symbolic paths and malformed provenance', t => {
  for (const invalid of ['../private.md', '/etc/passwd', 'C:/private.md', '.agents/skills/project-foundation/references/../../private.md']) {
    const destination = fixture(t);
    edit(destination, m => { m.files[invalid] = hash('secret'); });
    assert.equal(previewUpdate(destination).status, 'error');
  ...
  fs.symlinkSync(path.join(destination, 'moved'), skillRoot, process.platform === 'win32' ? 'junction' : 'dir');
  assert.equal(previewUpdate(destination).status, 'error');
  const malformed = fixture(t);
  edit(malformed, m => { m.provenance.packageVersion = {}; });
  assert.equal(previewUpdate(malformed).status, 'error');
});
test('update-preview JSON and CLI exit codes', t => {
  const destination = fixture(t);
  const cli = args => spawnSync(process.execPath, ['dist/cli.js', 'update-preview', '--dest', destination, ...args], { encoding: 'utf8' });
  let result = cli(['--json']);
