...
The GitHub command downloads the package through npm; the installer itself makes no network requests and has no runtime dependencies. For reproducible installs, append `#<reviewed-commit-sha>` to the GitHub package spec. `npx devmethod init` will only be available after publication of this package on npm; it is not the current installation command.
