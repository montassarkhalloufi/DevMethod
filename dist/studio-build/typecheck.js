...
import { ALLOWED_PACKAGES, problem } from './types.js';
import { normalize, packageName, trustedRequire } from './resolution.js';
const options = {
...
function moduleFile(id, containing, context, host) {
...
function compilerHost(context) {
    ...
    host.directoryExists = (directory) => {
        ...
        return [...context.files.keys(), ...context.roots.map((root) => normalize(root) + '/')].some((file) => file.startsWith(dir) || dir.startsWith(file));
    };
    ...
    host.resolveModuleNames = (names, containing) => names.map((id) => moduleFile(id, containing, context, host));
    return host;
}
function sourceRestrictions(source, context) {
    const diagnostics = [];
    const file = path.relative(context.sourceRoot, source.fileName).replaceAll('\\', '/');
    function visit(node) {
        if (node.kind === ts.SyntaxKind.AnyKeyword) {
            diagnostics.push({
                ...problem('Un type any explicite contourne le profil strict ; utiliser unknown puis affiner le type.', file),
        ...
        if (ts.isCallExpression(node) &&
            ...
            diagnostics.push(problem('Les imports dynamiques doivent utiliser un chemin littéral connu.', file));
        }
...
function suppressionComments(source, file) {
    const diagnostics = [];
    const scanner = ts.createScanner(ts.ScriptTarget.Latest, false, ts.LanguageVariant.JSX, source.text);
    for (let token = scanner.scan(); token !== ts.SyntaxKind.EndOfFileToken; token = scanner.scan()) {
        ...
        if (!/@ts-(?:ignore|nocheck|expect-error)\b/.test(scanner.getTokenText()))
            continue;
        diagnostics.push({
            ...problem('Les directives qui suppriment le contrôle TypeScript sont refusées dans les sources applicatives.', file),
            line: source.getLineAndCharacterOfPosition(scanner.getTokenPos()).line + 1,
        });
    }
    return diagnostics;
}
export function typecheck(context) {
    const sourceDirectory = normalize(path.join(context.sourceRoot, 'src')) + '/';
    const rootNames = [...context.files.keys()].filter((file) => file.startsWith(sourceDirectory) && /\.(ts|tsx)$/.test(file));
    ...
    const diagnostics = ts
        ...
        .slice(0, 100)
        .map((item) => {
        const file = item.file && context.files.has(normalize(item.file.fileName))
