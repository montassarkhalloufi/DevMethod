import type { RiskAnalysisRun } from './hybrid-contracts.js';
...
function requireValid(value: unknown): asserts value {
  if (!value) throw new Error('Journal d’analyse de risque invalide.');
}
...
export function validateRiskOutput(value: unknown) {
  record(value);
  ...
  for (const finding of value.findings) {
    ...
    requireValid(
      categories.includes(String(finding.category)) &&
...
export function validateRiskRuns(value: unknown): asserts value is RiskAnalysisRun[] {
  requireValid(Array.isArray(value) && value.length <= 100);
  const ids = new Set();
  for (const run of value) {
    record(run);
    for (const key of ['id', 'contextKey', 'revisionId', 'provider']) requireValid(text(run[key]));
    requireValid(!ids.has(run.id));
    ids.add(run.id);
    requireValid(
      ['running', 'completed', 'failed', 'cancelled', 'interrupted'].includes(String(run.status)),
    );
    requireValid(Number.isFinite(Date.parse(String(run.startedAt))));
    if (run.status !== 'running') requireValid(Number.isFinite(Date.parse(String(run.finishedAt))));
    if (run.status === 'completed') {
      requireValid(run.usage != null);
      validateRiskOutput(run.output);
    } else requireValid(run.output === undefined);
    if (run.error !== undefined) requireValid(text(run.error));
    validateUsage(run.usage);
  }
  requireValid(value.filter((run) => run.status === 'running').length <= 1);
}
export function validateRiskRunTransition(
  previous: RiskAnalysisRun[] = [],
  next: RiskAnalysisRun[] = [],
) {
  requireValid(next.length >= previous.length);
  previous.forEach((old, index) => {
    const current = next[index];
    requireValid(current);
    if (old.status !== 'running') requireValid(JSON.stringify(old) === JSON.stringify(current));
    else
      for (const key of ['id', 'contextKey', 'revisionId', 'provider', 'startedAt'] as const)
        requireValid(old[key] === current[key]);
  });
  for (const run of next.slice(previous.length)) requireValid(run.status === 'running');
}
