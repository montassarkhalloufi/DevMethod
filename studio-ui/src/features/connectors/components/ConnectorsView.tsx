    ...
    <div className="connectors-view">
      <div className="connector-toolbar">
        ...
        <button type="button" className="connector-refresh" onClick={refresh} disabled={busy}>
          {connectorText('Actualiser les états', locale)}
        </button>
      </div>
      {error ? (
      ...
      {!report ? (
        <p role="status">
          {error
            ? connectorText('Aucun état de connexion confirmé.', locale)
            : connectorText('Lecture du catalogue…', locale)}
        </p>
      ) : null}
      ...
      {report ? (
        <details className="connector-report-limits">
          <summary>{connectorText('Ce que DevMethod prend en charge', locale)}</summary>
          <ul>
