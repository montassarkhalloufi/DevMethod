...
import type { Review } from './review-model.js';
...
function validateOptions(options: ReviewOptions): void {
  for (const value of [
    ...
    if (value !== undefined && !value.trim()) throw new Error('Review options must not be empty.');
  if (options.currentRevision && options.currentRevision.length > 8192)
    throw new Error('Revision label exceeds the allowed length.');
  if (
    options.changedTargets &&
    (options.changedTargets.length > 256 ||
      options.changedTargets.some((t) => !t.trim() || t.length > 8192))
  )
...
function validateOutputs(root: string, outputs: [string, string][], options: ReviewOptions): void {
  for (const [file] of outputs) {
    if (
      !safePath(file) ||
      secretPath(file) ||
      (file === options.output ? !file.endsWith('.html') : !file.endsWith('.md'))
    )
...
function writeOutputs(root: string, outputs: [string, string][]): void {
  ...
  try {
    ...
    for (const f of created.reverse()) fs.unlinkSync(f);
    for (const d of directories.reverse()) fs.rmdirSync(d);
    throw error;
  }
}
...
export function prepareReview(options: ReviewOptions) {
  ...
  const read = (file: string): string => {
    if (secretPath(file)) throw new Error('Secret-like review paths are excluded.');
    return readLocal(root, file, 4 * 1024 * 1024).toString('utf8');
  };
  const source = options.demo
    ? fs.readFileSync(
        fileURLToPath(new URL('../examples/review/review.json', import.meta.url)),
        'utf8',
      )
    : options.review
      ? read(options.review)
  ...
  if (options.output)
    outputs.push([
      ...
      renderReviewHTML({
        ...
        changedTargets: options.changedTargets,
      }),
    ]);
  if (options.markdown) outputs.push([options.markdown, review ? reviewMarkdown(review) : legacy!]);
  validateOutputs(root, outputs, options);
  writeOutputs(root, outputs);
  return reviewResult(root, options, review, legacy, outputs);
}
...
function reviewResult(
  ...
  const summary = review ? summarizeReview(review) : null;
  return {
    format: 1,
    reviewId: review?.id ?? null,
    status: summary ? summary.conclusion : legacy !== null ? 'legacy' : 'empty',
    summary,
    freshness: review
      ? reviewFreshness(review, options.currentRevision ?? null, options.changedTargets ?? [])
      : null,
    outputs: outputs.map(([file]) => path.resolve(root, file)),
    limitations:
      'Explicit record only; no review execution or filesystem evidence discovery. Legacy Markdown has no inferred fields. Inspect privacy before sharing; automatic redaction is heuristic.',
  };
}
