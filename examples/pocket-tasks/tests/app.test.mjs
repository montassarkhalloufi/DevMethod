...
async function fixture(t) {
  ...
  async function start() {
    const server = createTaskServer({ dataFile: file });
    servers.push(server);
    ...
    return {
      server,
      call: (path = '/api/tasks', method = 'GET', payload) =>
        fetch(`${base}${path}`, {
          method,
...
test('domain normalizes titles and rejects invalid shapes/fields', () => {
  ...
  for (const body of [
    ...
    assert.throws(() => validateBody(body, true));
...
test('HTTP CRUD and restart preserve titles, completion and deletion', async (t) => {
  ...
  const updated = await f.call(`/api/tasks/${task.id}`, 'PATCH', {
    title: ' Revised ',
    done: true,
  });
  assert.equal(updated.status, 200);
  assert.deepEqual((await updated.json()).task, { id: task.id, title: 'Revised', done: true });
  await new Promise((resolve) => f.server.close(resolve));
  const restarted = await f.start();
  assert.deepEqual((await (await restarted.call()).json()).tasks, [
    { id: task.id, title: 'Revised', done: true },
  ]);
  assert.equal(
    (await restarted.call(`/api/tasks/${task.id}`, 'PATCH', { done: false })).status,
    200,
  );
...
test('bad HTTP input and oversized body never change persisted state', async (t) => {
  ...
  for (const payload of [
    ...
    const response = await f.call('/api/tasks', 'POST', payload);
    assert.equal(response.status, 400);
    assert.equal(typeof (await response.json()).error, 'string');
  }
  for (const payload of [{}, { done: 1 }, { title: null }, { extra: 'value' }])
    assert.equal((await f.call(`/api/tasks/${task.id}`, 'PATCH', payload)).status, 400);
  const large = await f.call('/api/tasks', 'POST', { title: 'x'.repeat(17000) });
...
test('concurrent mutations serialize without losing tasks or leaking temp files', async (t) => {
  ...
  const responses = await Promise.all(
    Array.from({ length: 20 }, (_, n) => f.call('/api/tasks', 'POST', { title: `Task ${n}` })),
  );
...
test('malformed persisted data fails closed and recovers only after explicit repair', async (t) => {
  ...
  for (const malformed of [
    ...
    '[{"id":"x","title":"hello","done":"yes"}]',
    '[{"id":"x","title":"hello","done":false},{"id":"x","title":"again","done":true}]',
  ]) {
...
test('static allowlist serves UI and rejects private assets and unknown methods', async (t) => {
  ...
  const js = await (await f.call('/app.js')).text();
...
test('local HTTP boundary rejects cross-site writes, rebinding hosts and simple MIME without mutation', async (t) => {
  ...
  const send = (method, path, headers, payload = '{}') =>
  ...
  for (const mime of [
    ...
    for (const [method, path] of [
      ...
      assert.equal(
        (await send(method, path, { ...local, 'Content-Type': mime }, '{"title":"Injected"}'))
          .status,
        415,
      );
    }
  ...
  assert.equal(
    (await send('GET', '/api/tasks', { ...local, Host: `attacker.example:${port}` })).status,
    403,
  );
  assert.equal(await readFile(f.file, 'utf8'), before);
  assert.equal(
