...
- Read [verification and cost](references/verification-and-cost.md) for review criteria, tool-result handling and costs. When a check fails, is unavailable, progress stalls or an execution is interrupted, apply [bounded correction](references/bounded-correction.md); retain the diagnosis in the existing ticket or inline Quick record. Use its feedback guidance when method improvement is authorized.
