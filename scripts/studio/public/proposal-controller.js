...
export function createProposalController({
  ...
  const actions = {
    async select(proposalId, optionId) {
      ...
      if (!result)
        throw new Error('La sélection n’a pas été enregistrée. Vérifiez le message du Studio.');
    ...
    async approve(proposalId, optionId, reason) {
      const result = await change('proposals/approve', {
        proposalId,
        optionId,
        reason: reason.trim() || 'Option approuvée explicitement ; aucune justification ajoutée.',
      });
      if (!result)
        throw new Error('La décision n’a pas été enregistrée. Votre raison est conservée.');
    },
  };
