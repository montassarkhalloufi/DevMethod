  ...
  dom.window.fetch = async (url, init) => {
    ...
    try {
      if (init?.method === 'POST') {
        ...
        if (state.fail && url !== '/api/connectors/guides/prepare?language=fr')
          throw new Error('fixture offline');
      }
      let result;
      if (url === '/api/connectors/guides?language=fr') result = readConnectorGuides('fr');
      else if (url === '/api/connectors/guides/prepare?language=fr')
        result = prepareConnectorGuide(input);
