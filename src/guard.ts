...
import { commandGate, type GuardCommand } from './commands.js';
import { inspectAcceptance } from './closure.js';
import { behavioralReceipt, evaluateBehavior } from './behavior.js';
import { captureGuardSnapshot, type BehaviorInputs } from './guard-context.js';
import { halt, haltReason, type GuardState } from './guard-state.js';
import { withGuardStore, type GuardStore } from './guard-store.js';
...
const limitations =
  'Local guard only: no host interception, agent dispatch, deployment or merge. Frozen context covers declared files; external writers must be quiescent. Hashes bind bytes, not authenticity or semantic truth. Session storage is trusted local state.';
...
function newState(options: GuardOptions): GuardState {
  return {
    format: 1,
    root: options.root,
...
function gate(options: GuardOptions, state: GuardState | null): string | null {
...
function verificationInputs(options: GuardOptions): BehaviorInputs {
...
function verify(options: GuardOptions, state: GuardState, store: GuardStore) {
  const inputs = verificationInputs(options);
  const before = captureGuardSnapshot(options.root, inputs);
  if (
    state.receipt?.signature === behavioralReceipt(before).signature &&
...
function integrate(options: GuardOptions, state: GuardState, store: GuardStore) {
  if (!options.checkpointPath) throw new Error('Integration requires --checkpoint.');
  // Invalidate a previous acceptance before assessment; a crash cannot retain acceptance.
  state.featureState = 'verification-failed';
  store.save(state);
  const acceptance = inspectAcceptance(
    options.root,
    options.missionPath,
    options.checkpointPath,
    state.receipt,
  );
  if (acceptance.allowed) state.featureState = 'accepted';
  else state.receipt = null;
  store.save(state);
  return report(
    state,
    acceptance.allowed ? 'accepted' : 'blocked',
    acceptance.allowed,
    acceptance.reason,
  );
}

function execute(options: GuardOptions, store: GuardStore) {
  ...
  if (!state) throw new Error('Implementation gate required.');
  return options.command === '/verify'
    ? verify(options, state, store)
    : integrate(options, state, store);
}

export function runGuard(options: GuardOptions) {
  try {
    const root = path.resolve(options.root);
    checkPath(root);
    const normalized = { ...options, root: fs.realpathSync(root) };
    return withGuardStore(
      normalized.root,
      options.session,
      options.command === '/implement',
      (store) => executeSafely(normalized, store),
    );
  } catch {
...
function executeSafely(options: GuardOptions, store: GuardStore) {
  try {
