...
import { presentation as localizedPresentation } from '../scripts/studio/public/version-presentation.js';
import { createInitialStudioState } from '../scripts/studio/store.mjs';

const presentation = (state, options) => localizedPresentation(state, options, 'fr');
...
test('delegation, master approval, selected direction and passing checks do not approve a revision render', () => {
  ...
  assert.equal(result.visual.approval, null);
  assert.equal(result.visual.status, 'pending');
  assert.equal(result.checks.passed, 0);
...
test('version projection defaults to English and translates only authored labels', () => {
  const state = fixture();
  approve(state);
  const options = { displayedRevisionId: 'candidate-version' };
  const english = localizedPresentation(state, options);
  assert.match(english.displayed.label, /^Unapplied proposal · Formulaire direct$/);
  assert.equal(english.visual.label, 'Visuals accepted for this version · approved by you');
  const french = localizedPresentation(state, options, 'fr');
  assert.match(french.displayed.label, /^Proposition non appliquée · Formulaire direct$/);
  assert.equal(english.proposal.question, 'Quel formulaire ?');
  assert.equal(english.visual.approval.reason, 'Rendu examiné');
  assert.deepEqual(english.visual.approval, french.visual.approval);
});
