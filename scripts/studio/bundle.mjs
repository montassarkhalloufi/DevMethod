...
import { readQualityRuns } from './quality-storage.mjs';
...
export function exportProject(workspace, state) {
  ...
  // Receipts travel with the project. Machine-local launch permission and profiles do not.
  for (const run of readQualityRuns({ root: workspace })) {
    if (!/^[a-f0-9-]{36}$/.test(run.id)) throw new Error('Identifiant de preuve qualité invalide.');
    add(`.devmethod/quality/${run.id}.json`, JSON.stringify(run, null, 2) + '\n');
  }
