...
A failed `/verify` returns to the relevant correction. Preserve the observed failure, likely cause, next distinguishing check or changed approach, and applicable attempt limit in the existing task record. A repeated failure without new information calls for diagnosis or reassessment, not an identical relaunch. This is a workflow instruction, not an automatic retry controller. If a gate is blocked by the environment, recommend `/correct-course` or `/handoff`, not `/integrate`. An explicit `/integrate` invocation with an unsatisfied gate may prepare a candidate, but must refuse acceptance. `/integrate` respects and names the actual authorized delivery mode (local, PR, or merge). At scope completion, `/next` records completion and offers `/status` as an optional consultation; it does not create new features or an automatic loop.

At a stop or handoff, name the reason: objective verified, blocked dependency, limit reached, interruption, or user-ended scope. An exhausted budget or a successful process exit is not objective completion. Preserve the exact authorized next action when one exists; a completed or user-ended mission does not generate another task.

## Optional local guard

When the project explicitly adopts `devmethod guard`, its `/implement`, `/verify` and `/integrate` gates enforce the local session contract in `docs/ADR-012-local-execution-guard.md`. Two consecutive identical behavioral failure signatures halt that session, preserve its declared context and require human reconciliation. Integration must record a current verified behavioral evidence signature with criterion coverage before local acceptance. The guard does not dispatch agents, register bare host slash commands, merge, deploy or replace human authorization; ordinary host-guided work and the read-only inspectors retain their existing contracts.
